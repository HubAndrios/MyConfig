...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	ТекстЗапроса =
		...
		|		ВЫБРАТЬ
		|			Таблица.Назначение                  КАК Назначение,
		|			ВЫБОР КОГДА НазначенияБезЗаказа.Ссылка ЕСТЬ NULL ТОГДА
		|						ЛОЖЬ
		|					ИНАЧЕ
		|						ИСТИНА
		...
		|				
		|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК НазначенияБезЗаказа
		|				ПО Таблица.Назначение = НазначенияБезЗаказа.Ссылка
		|				 И(НазначенияБезЗаказа.Заказ = НЕОПРЕДЕЛЕНО
		|				)
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		|		
