...
Функция СообщениеУдовлетворяетУсловиюОтправки(ДанныеВидаОповещения, СтрокаДанныхШаблона, ДанныеПолучателей)
	...
	Попытка
		...
		Если ОбщегоНазначения.ВычислитьВБезопасномРежиме(УсловиеОтправки) = Ложь Тогда
			Возврат Ложь;
		Иначе
			Возврат Истина;
		КонецЕсли;
		...
		Попытка
			...
			Если ПараметрыУсловия <> Неопределено Тогда
				Для Каждого Параметр Из ПараметрыУсловия Цикл
					Если (Не Параметр.ЭтоПараметрПредущегоСообщения) И (ЭлементСоответствия.Значение.Получить(Параметр.ИмяПараметра) = Неопределено) Тогда
