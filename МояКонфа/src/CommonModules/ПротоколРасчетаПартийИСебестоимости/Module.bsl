...
#Область ЗаголовокПротокола
	...
	Если ПараметрыРасчета.ФормироватьНачальныеОстаткиПартий22 Тогда
		Протокол.ДобавитьСтроку(НСтр("ru='Заменить суммовые остатки регистра себестоимости остатками партионных регистров:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.НачальныеОстатки.ВзятьОстаткиСебестоимостиИзРегистровПартий));
		Протокол.ДобавитьСтроку(НСтр("ru='Распределить расхождения суммовых остатков партий и себестоимости на партии:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.НачальныеОстатки.РаспределятьРасхожденияВСуммахПартийИСебестоимости));
	КонецЕсли;
	...
	Для Каждого ТекущаяФО Из ПараметрыРасчета.ФО Цикл
		Протокол.ДобавитьСтроку("	- " + ТекущаяФО.Ключ + ": "	+ ПредставлениеЗначения(ТекущаяФО.Значение));
	КонецЦикла;
	
	Протокол.ДобавитьСтроку("");
	Протокол.ДобавитьСтроку(РазделительЧастейПротокола());
	Протокол.ДобавитьСтроку(НСтр("ru='Ограничения выборки данных:'"));
	
	Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. количество строк в таблице значений:'") + " "
		+ ПредставлениеЗначения(ПараметрыРасчета.ОграниченияВыборки.КоличествоСтрокВТЗ));
	Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. количество строк в наборе записей:'") + " "
		+ ПредставлениеЗначения(ПараметрыРасчета.ОграниченияВыборки.КоличествоЗаписейВНЗ));
	
	Если ПараметрыРасчета.ЗапущенРасчетПартий Тогда
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Размер порции данных в механизме распределения партий:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.ОграниченияВыборки.ПорцияРасчетаПартий));
	КонецЕсли;		
	
	Протокол.ДобавитьСтроку("");
	Протокол.ДобавитьСтроку(РазделительЧастейПротокола());
	Протокол.ДобавитьСтроку(НСтр("ru='Параметры решения СЛУ:'"));
	
	Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. допустимое отклонение при решении СЛУ:'") + " "
		+ ПредставлениеЗначения(ПараметрыРасчета.РешениеСЛУ.ТребуемаяТочность));
	Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. количество итераций решения СЛУ:'") + " "
		+ ПредставлениеЗначения(ПараметрыРасчета.РешениеСЛУ.МаксимальноеКоличествоИтераций));
	Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. сумма, считаемая ошибкой округления:'") + " "
		+ ПредставлениеЗначения(ПараметрыРасчета.РешениеСЛУ.ЗначениеПогрешностиСтоимость));
	
	Если ПараметрыРасчета.ОграниченияФЗ.МаксимальноеКоличествоФЗ > 1 Тогда
		
		Протокол.ДобавитьСтроку("");
		Протокол.ДобавитьСтроку(РазделительЧастейПротокола());
		Протокол.ДобавитьСтроку(НСтр("ru='Параметры многопоточной записи движений:'"));
		
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. количество потоков записи движений:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.ОграниченияФЗ.МаксимальноеКоличествоФЗ));
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Макс. количество движений, записываемое фоновым заданием:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.ОграниченияФЗ.КоличествоДвиженийВФЗ));
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Последовательная запись порций движений регистра:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.ОграниченияФЗ.ПоследовательнаяЗаписьРегистра));
			
	КонецЕсли;
	
	Протокол.ДобавитьСтроку("");
	Протокол.ДобавитьСтроку(РазделительЧастейПротокола());
	Протокол.ДобавитьСтроку(НСтр("ru='Параметры управления итогами регистров:'"));
		
	Протокол.ДобавитьСтроку("	- " + НСтр("ru='Запись движений с включенными итогами по регистру:'") + " "
		+ ПредставлениеЗначения(НЕ ПараметрыРасчета.УправлениеИтогами.ОтключатьИтогиПередЗаписью));
		
	// Параметры отладки
	УстановленыПараметрыОтладки = Ложь;
	Для Каждого КлючИЗначение Из ПараметрыРасчета.Отладка Цикл
		Если (ТипЗнч(КлючИЗначение.Значение) = Тип("Булево") И КлючИЗначение.Значение)
		 ИЛИ (ТипЗнч(КлючИЗначение.Значение) <> Тип("Булево") И ЗначениеЗаполнено(КлючИЗначение.Значение)) Тогда
			УстановленыПараметрыОтладки = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если УстановленыПараметрыОтладки Тогда
		
		Протокол.ДобавитьСтроку("");
		Протокол.ДобавитьСтроку(РазделительЧастейПротокола());
		Протокол.ДобавитьСтроку(НСтр("ru='Отладочные параметры:'"));
		
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Очищать все старые расчетные движения:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.Отладка.ОчищатьСтарыеДвижения));
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Контролировать корректность формирования порций многопоточной записи:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.Отладка.КонтролироватьРегистраторыВФЗ));
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Исправлять пустой регистратор:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.Отладка.ИсправлятьПустойРегистратор));
		Протокол.ДобавитьСтроку("	- " + НСтр("ru='Каталог сохранения данных СЛУ:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.Отладка.КаталогДляСохраненияДанныхСЛУ));
			
		Если ПараметрыРасчета.ЗапущенРасчетПартий Тогда
			Протокол.ДобавитьСтроку("	- " + НСтр("ru='Игнорировать ошибки в первичных движениях документов:'") + " "
				+ ПредставлениеЗначения(ПараметрыРасчета.Отладка.ИгнорироватьНекорректныеПервичныеДвижения));
		КонецЕсли;
...
#Область ИтоговыеПоказатели
	...
	Если ПараметрыРасчета.ОбщееКоличествоФЗ > 0 Тогда
		...
		Протокол.ДобавитьСтроку(НСтр("ru='Всего запущено фоновых заданий записи движений:'") + " "
			+ ПредставлениеЗначения(ПараметрыРасчета.ОбщееКоличествоФЗ));
...
//
Процедура ВывестиИнформациюОбОшибках(ПараметрыРасчета, Протокол, ДанныеОшибок, Заголовок)
...
Процедура НачатьЗамерВремени(ПараметрыРасчета) Экспорт
	...
	ИначеЕсли ПараметрыРасчета.ВариантРасчета = Перечисления.ВариантыРасчетаПартийИСебестоимости.ПартииИСебестоимость Тогда
		ОписаниеЗамераПроизводительности = Производительность.НачатьЗамерВремени("ЗакрытиеМесяца.РасчетПартийИСебестоимости22");
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ЗакончитьЗамерВремени(ПараметрыРасчета) Экспорт
	...
	Если ПараметрыРасчета.Свойство("ОписаниеЗамера") Тогда
		...
		ЗафиксироватьЗамерВремени(ПараметрыРасчета);
	Иначе
		Производительность.ЗакончитьЗамерВремени(
...
Процедура ЗафиксироватьЗамерВремени(ПараметрыРасчета) Экспорт
	...
	Производительность.ЗафиксироватьЗамерВремени(
		ПараметрыРасчета.Производительность.ОписаниеЗамераПроизводительности,
		ПараметрыРасчета.Производительность.КоличествоДанных);
