...
// 
//  Если доступа к одному из реквизитов нет, то в случае "ВыбратьРазрешенные = Ложь" возникнет
//  исключение прав доступа, а в случае "ВыбратьРазрешенные = Истина" вернется значение Неопределено для всех
//  реквизитов.
//
//  Если необходимо зачитать реквизит независимо от прав текущего пользователя,
...
//  ВыбратьРазрешенные - Булево - если Истина, то запрос к объектам выполняется с учетом прав пользователя, и в случае,
//                                если какой-либо объект будет исключен из выборки по правам, то значение реквизита
//                                этого объекта вернется со значением Неопределено;
//                                если Ложь, то возникнет исключение при отсутствии прав на любой из реквизитов любого
//                                из объектов.
// 
// Возвращаемое значение:
...
Процедура УстановитьСвойстваДинамическогоСписка(Список, СтруктураПараметров) Экспорт
	...
	Если РежимОтладки Тогда
		Язык = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		РасположениеСписка = СтрШаблон("%1.%2", Форма.ИмяФормы, Список.ПутьКДанным);
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ТекстЗапроса) Тогда
		...
		Если РежимОтладки Тогда
			ИмяСобытия = НСтр("ru = 'Установка свойств динамического списка.Текст запроса'", Язык);
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация,, РасположениеСписка, ТекстЗапроса);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ОсновнаяТаблица) Тогда
		...
		Если РежимОтладки Тогда
			ИмяСобытия = НСтр("ru = 'Установка свойств динамического списка.Основная таблица'", Язык);
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация,, РасположениеСписка, ОсновнаяТаблица);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(ДинамическоеСчитываниеДанных) = Тип("Булево") Тогда
		...
		Если РежимОтладки Тогда
			ИмяСобытия = НСтр("ru = 'Установка свойств динамического списка.Динамическое считывание данных'", Язык);
...
// Помещает произвольные данные во внутренний буфер обмена.
//
// Параметры:
...
// Получает строки табличной части, помещенные во внутренний буфер обмена.
//
// Возвращаемое значение:
//  Структура - со свойствами:
