...
//	- нестандартно расчитываются кэши остатков и оборотов
...
Функция РассчитатьВсеВПопыткеИсключенииПриПереходеНаВерсию241(ПараметрыЗапуска) Экспорт
	...
	Попытка
		РасчитатьВсеПриПереходеНаВерсию241(ПараметрыЗапуска);
	Исключение
...
//
Процедура РасчитатьВсеПриПереходеНаВерсию241(ПараметрыЗапуска) Экспорт
	...
	Для Каждого СтрокаСхемыРасчета Из СхемаРасчета Цикл
		...
		//		- добавим к ним непересчитываем ресурсы из ИБ
...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|		ВТДвиженияСебестоимостьТоваров КАК Т
	|	ГДЕ
	|		ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ПоступлениеОтПереработчика)
	|		И Т.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|
	|	) КАК Т
	|
	...
	РасшифровкаКодовОшибок.Вставить(14, НСтр("ru='Для документов переработки на стороне указаны неправильные разделы учета'"));
	...
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.ПереоценкаВалютныхСредств"));
...
Функция ТекстНачальныеОстаткиНЗП() // Новая функция. Добавить.
	Возврат 
		"ВЫБРАТЬ
	...
	Если Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки
	 ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки Тогда
		...
		Если ЗначениеЗаполнено(Приход.КорРазделУчета) Тогда
			...
			ОтклоненияНаСклад.НДСУпр = Отклонения.НДСУпр;
...
Процедура ПолучитьДанныеДляПартийВыручки(ПараметрыРасчета)
	...
	Запрос.Текст = ""
		...
		+ ТекстОбъединениеЗапросов() + ТекстВыручкаПоУслугам()
		+ "";
...
Функция ТекстВыручкаПоУслугам()
	Возврат 
		"ВЫБРАТЬ
		|	""ТекстВыручкаПоУслугам"" 						  	  КАК ЗапросИсточник,
...
Функция ПолучитьДанныеДляДопРасходов(ПараметрыРасчета)
	...
	УниверсальныеМеханизмыПартийИСебестоимости.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		...
		|Декларации, КоличествоДеклараций,
...
Процедура СформироватьДопРасходыВПрочиеРасходы(ПараметрыРасчета, РасчетныеПриходыПартийРасходов)
	...
	Для Каждого Движение Из РасчетныеПриходыПартийРасходов Цикл
		...
		Если Движение.ВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД
		 ИЛИ Движение.ВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
...
Функция ТекстБазисПеремещенияСебестоимость()
	...
	ТекстЗапроса = 
		...
		|	СУММА(ВЫРАЗИТЬ(&Вес КАК ЧИСЛО(15,3)) * ДД.Количество) КАК Вес,
		|	СУММА(ВЫРАЗИТЬ(&Объем КАК ЧИСЛО(15,3)) * ДД.Количество) КАК Объем,
		|	СУММА(ВЫРАЗИТЬ(ДД.Количество * ЕСТЬNULL(Цены.Стоимость, 0) КАК ЧИСЛО(15,3))) КАК Стоимость,
		|	СУММА(ВЫРАЗИТЬ(ДД.Количество * ЕСТЬNULL(Цены.СтоимостьБезНДС, 0) КАК ЧИСЛО(15,3))) КАК СтоимостьБезНДС,
		|	СУММА(ВЫРАЗИТЬ(ДД.Количество * ЕСТЬNULL(Цены.СтоимостьРегл, 0) КАК ЧИСЛО(15,3))) КАК СтоимостьРегл,
		|	0 КАК НДСРегл,
		|	0 КАК НДСУпр,
		|	СУММА(ДД.ПостояннаяРазница) КАК ПостояннаяРазница,
		|	СУММА(ДД.ВременнаяРазница) КАК ВременнаяРазница,
...
Функция ТекстРаспределениеРасходовНаВыбытиеТоваровПрошлыхПериодов() // ВТ РаспределениеРасходов - общая для регистров себестоимости, выручки, прочих расходов
	...
	|	ИЛИ (ДД.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ДопРасходы) И ПрошлыеВыбытия.ЭтоОстатки)
...
Процедура ПолучитьДанныеДляПартийНДС(ПараметрыРасчета)
	...
	УниверсальныеМеханизмыПартийИСебестоимости.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		"ПриходыТоваров, СписаниеОтклоненийВСтоимости, ОстаткиТоваров, ПрошлыеРеализацииПартийНДС, ПриходыТоваровНДС, ДвиженияСебестоимостиНДС");
