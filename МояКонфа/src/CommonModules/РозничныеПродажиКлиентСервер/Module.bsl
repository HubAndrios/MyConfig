...
// Возвращает данные для передачи в ЕГАИС
//
// Параметры:
//  МассивСтрок	 - Массив - Строки табличной части Товары
//  Поля		 - Структура - Описание разрядности для числовых полей
// 
// Возвращаемое значение:
//  Массив - Данные для передачи в ЕГАИС
//
Функция ПодготовитьДанныеЕГАИС(МассивСтрок, Поля) Экспорт
	...
	КонецЦикла;
	Реквизиты = СтрСоединить(МассивПолей, ",");
	...
	Для Каждого СтрокаТЧ Из МассивСтрок Цикл
		...
		Если СтрокаТЧ.МаркируемаяАлкогольнаяПродукция Тогда
			...
			Если ТипЗнч(СтрокаТЧ.КодАкцизнойМарки) = Тип("Массив")
				И СтрокаТЧ.КодАкцизнойМарки.Количество() = СтрокаТЧ.Количество Тогда
			
				КодыАкцизныхМарок = СтрокаТЧ.КодАкцизнойМарки;
				
				СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.СтруктураПересчетаСуммы(
					Реквизиты,, "Количество", Поля);
					
				ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьСтруктуруПересчетаСуммы(СтруктураПересчетаСуммы, СтрокаТЧ);
				
				Для Индекс = 1 По СтрокаТЧ.КоличествоУпаковок Цикл
					
					НоваяСтрока = ИнтеграцияЕГАИСКлиентСервер.СтруктураСтрокиЧекаЕГАИС();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ);
					
					НоваяСтрока.Количество = 1;
					НоваяСтрока.КодАкцизнойМарки = КодыАкцизныхМарок[СтрокаТЧ.КоличествоУпаковок - Индекс];
			...
			ИначеЕсли ТипЗнч(СтрокаТЧ.КодАкцизнойМарки) = Тип("Строка")
				И СтрокаТЧ.Количество = 1 Тогда
				
				Результат.Добавить(СтрокаТЧ);
				
			Иначе
				
				ВызватьИсключение НСтр("ru = 'Количество акцизных марок не соответствует общему количеству алкогольной продукции.'");
