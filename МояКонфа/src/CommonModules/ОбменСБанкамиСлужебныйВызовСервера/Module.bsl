...
//
Функция НастройкаОбменаПоСертификату(Сертификат) Экспорт
...
//  Банк  - СправочникСсылка.КлассификаторБанков - банк в настройке.
//
// Возвращаемое значение:
...
//
// Параметры:
//   Банк - СправочникСсылка.КлассификаторБанков - банк, для которого идет поиск настроек;
...
Функция Фрод(Параметры) Экспорт
	...
	ЕстьОшибка = Ложь;
	ЗаполнитьФродПараметрыВXDTO(ФродЭл, Параметры, ЕстьОшибка);
...
//                  Неопределено - журналирование отключено;
//  ЕстьОшибка - Булево - признак возникновения ошибки при чтении ответа банка.
//
Процедура ОбработатьОтветСбербанка(Знач Ответ, Знач НастройкаОбмена, МассивНовыхСообщений, Знач Идентификатор = Неопределено, ЕстьОшибка = Ложь) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Возникла ошибка при чтении полученных данных из банка.
									|%1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, КраткаяИнформация);
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru = 'Чтение полученных данных из банка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбработатьИзвещениеСОшибкойСбербанк(Знач Ответ, ТекстСообщения) Экспорт
	...
	Исключение
		...
		ТекстЗаписи = НСтр("ru = 'Возникла ошибка при чтении полученных данных из банка.
							|Ошибка: %1
							|Содержимое ответа: %2");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстЗаписи = СтрШаблон(ТекстЗаписи, ПодробноеПредставлениеОшибки, Ответ);

		ВидОперации = НСтр("ru = 'Чтение полученных данных из банка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстЗаписи, , "ОбменСБанками");
		Возврат
	КонецПопытки;
...
//
Функция ПакетXMLСбербанка(Знач СообщениеОбмена, Знач НастройкаОбмена, Знач ФродПараметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЭД.Тип() = ОбменСБанкамиСлужебный.ТипЗначенияCML(ФабрикаXDTO, ПространствоИмен, "PayDocRu") Тогда
			...
			Для Каждого ЭП Из МассивПодписей Цикл
				...
				ЕстьОшибка = Ложь;
				ЗаполнитьФродПараметрыВXDTO(Fraud, ФродПараметры, ЕстьОшибка);
				Если ЕстьОшибка Тогда
					Возврат Неопределено;
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		ДвоичныеДанныеФайла = ОбменСБанкамиСлужебный.ДвоичныеДанныеИзXDTO(ФабрикаXDTO, Request);
	Исключение
		ОбъектXML.Закрыть();
...
//
Функция НазваниеБизнесСистемыСбербанк(Знач НастройкаОбмена = Неопределено) Экспорт
...
//
// Возвращаемое значение:
//   Массив - содержит сформированные запросы выписок
//    * ДокументСсылка.СообщениеОбменСБанками - сформированное сообщение запроса выписки банка.
//
Функция ЗапросыВыписокСбербанк(Знач НастройкаОбмена, Знач ДатаНачала, Знач ДатаОкончания, Знач НомерСчета, ФормироватьПринудительно = Ложь, ГотовыеВыписки = Неопределено) Экспорт
	...
	Для Счетчик = 1 По КоличествоДней Цикл
		...
		Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
			...
			ДанныеЗапроса = СформироватьДанныеЗапросаВыпискиСбербанк(
				НастройкаОбмена, ДатаЗапроса, НомерСчета, ИдентификаторЗапроса);
				
			Если ДанныеЗапроса = Неопределено Тогда // произошла ошибка
				Возврат Неопределено;
			КонецЕсли;
...
Процедура ПолучитьЭДИзБанкаАсинхронныйОбмен(Знач НастройкаОбмена, Знач Параметры, ДанныеВозврата) Экспорт
	...
	Если Не Результат.Статус Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстОшибки, ТекстОшибки, "ОбменСБанками", НастройкаОбмена);
		ДанныеВозврата.ЕстьОшибка = Истина;
		Возврат;
	КонецЕсли;
...
Функция СформироватьДанныеЗапросаВыпискиСбербанк(НастройкаОбмена, ДатаЗапроса, НомерСчета, ИдентификаторЗапроса)
	...
	Попытка
		...
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(StmtReqType, "beginDate", НачалоДня(ДатаЗапроса), Истина, ТекстОшибки);
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(StmtReqType, "endDate", КонецДня(ДатаЗапроса), Истина, ТекстОшибки);
		...
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(Acc, "__content", НомерСчета, Истина, ТекстОшибки);
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(Acc, "bic", БИК,  Истина, ТекстОшибки);
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(Acc, "docNum", МенеджерЗаписи.НомерЗапроса, , ТекстОшибки);
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			ТекстСообщения = НСтр("ru = 'При формировании электронного документа произошла ошибка.
										|%1.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ТекстСообщения, ТекстСообщения, "ОбменСБанками", НастройкаОбмена);
			Возврат Неопределено;
		КонецЕсли;
		
		Возврат ОбменСБанкамиСлужебный.ДвоичныеДанныеИзXDTO(ФабрикаXDTO, StmtReqType);
...
Процедура СохранитьОтветСбербанка(ЭД, НастройкаОбмена, МассивНовыхСообщений)
	...
	Если ТипЗнч(ЭД.Tickets.Ticket) = Тип("СписокXDTO") Тогда
		Для Каждого Тикет Из ЭД.Tickets.Ticket Цикл
			ПрочитатьТикетСбербанка(НастройкаОбмена, Тикет, МассивНовыхСообщений);
		КонецЦикла;
	Иначе
...
Процедура СохранитьВыпискуСбербанка(НастройкаОбмена, ВыпискаXDTO, МассивВыписок)
	...
	Если Не ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ВыпискаXDTO, "Sign") = Неопределено Тогда
...
Процедура ПрочитатьТикетСбербанка(НастройкаОбмена, Тикет, МассивНовыхСообщений)
	...
	Если Выборка.Следующий() Тогда
		...
		Если НЕ ЗначениеЗаполнено(СтатусСББОЛ) ИЛИ ИзменилсяСтатусВКвитанцииСбербанк(Выборка.Владелец, СтатусСББОЛ) Тогда
			ДвоичныеДанныеФайлаОтвета = ОбменСБанкамиСлужебный.ДвоичныеДанныеИзXDTO(ФабрикаXDTO, Тикет);
			СсылкаНаХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайлаОтвета);
			...
			СтруктураРеквизитов.Вставить(
				"Идентификатор", ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Тикет, "docId"));
...
Процедура ЗаполнитьФродПараметрыВXDTO(ФродЭлемент, Параметры, ЕстьОшибка = Ложь)
	...
	ТекстОшибки = "";
	ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "DevicePrint", СтрокаФродКодированная, Истина, ТекстОшибки);
	
	Если ФродЭлемент.Свойства().Получить("ChannelIndicator") <> Неопределено Тогда
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "ChannelIndicator", "UPG_1C", Истина, ТекстОшибки);
	КонецЕсли;
	ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(
		ФродЭлемент, "IpMACAddresses", Параметры.IP + ";" + Параметры.MAC, Истина, ТекстОшибки);
	ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "GeolocationInfo", ";;;;4", Истина, ТекстОшибки);
	ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "PcProp", СвойстваКомпьютера, Истина, ТекстОшибки);
	ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "TokenInfo", TokenInfo, Истина, ТекстОшибки);
	КодЛокализации = КодЛокализацииИнформационнойБазы();
	ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "HttpAcceptLanguage", КодЛокализации, Истина, ТекстОшибки);
	
	Если ФродЭлемент.Свойства().Получить("Login") <> Неопределено Тогда
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(ФродЭлемент, "Login", ";", Истина, ТекстОшибки);
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		ШаблонСообщения = НСтр("ru = 'Ошибка аутентификации:
								|%1'");
		ТекстОшибки = СтрШаблон(ШаблонСообщения, ТекстОшибки);
		Операция = НСтр("ru = 'Аутентификация на сервере банка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстОшибки, "ОбменСБанками");
		ЕстьОшибка = Истина;
	КонецЕсли;
...
//
Функция МассивИдентификаторовЗапроса(Знач НастройкаОбмена, Знач ВидЭД, ИдентификаторОрганизации) Экспорт
