...
Процедура ПерезаполнитьКоличествоПоОрдерам(ТЧ, Регистратор, Распоряжения, Склад) Экспорт
	...
	Запрос = Новый Запрос(РегистрыНакопления.ТоварыКОформлениюПоступления.ТекстЗапросаОсталосьОформить(,Отбор));
...
Функция НастройкиФормыПереоформленияНакладных() Экспорт
	...
	НастройкиФормы.Вставить("ДанныеЗаполнения", Неопределено);
