...
Процедура СопоставитьАлкогольнуюПродукциюСНоменклатурой(Объект, ИмяКолонки = "НоменклатураЕГАИС") Экспорт
	...
	Данные = Объект.Товары.Выгрузить(, ИмяКолонки + ", " + "Номенклатура, Характеристика, Упаковка");
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Т.%ИмяКолонки% КАК НоменклатураЕГАИС,
	|	Т.Номенклатура,
	|	Т.Характеристика,
	|	Т.Упаковка
	|ПОМЕСТИТЬ Таблица
	...
	|	Таблица КАК Таблица
	|ГДЕ
	|	Таблица.НоменклатураЕГАИС <> ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка)
	|	И Таблица.НоменклатураЕГАИС.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)");
	...
	Пока Выборка.Следующий() Цикл
		...
		НоменклатураЕГАИСОбъект = Выборка.НоменклатураЕГАИС.ПолучитьОбъект();
		НоменклатураЕГАИСОбъект.Номенклатура   = Выборка.Номенклатура;
		НоменклатураЕГАИСОбъект.Характеристика = Выборка.Характеристика;
		
		Если НоменклатураЕГАИСОбъект.Упаковки.Количество() = 0 Тогда
			НоменклатураЕГАИСОбъект.Упаковки.Добавить();
		КонецЕсли;
		
		Упаковка = Выборка.Упаковка;
		Если НЕ ЗначениеЗаполнено(Упаковка) Тогда
			Упаковка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Номенклатура, "ЕдиницаИзмерения");
		КонецЕсли;
		
		СтрокаУпаковок = НоменклатураЕГАИСОбъект.Упаковки.Найти(Упаковка, "Упаковка");
		Если СтрокаУпаковок = Неопределено Тогда
			СтрокаУпаковок = НоменклатураЕГАИСОбъект.Упаковки.Найти(Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), "Упаковка");
			Если СтрокаУпаковок <> Неопределено Тогда
				СтрокаУпаковок.Упаковка = Упаковка;
			КонецЕсли;
		КонецЕсли;
		
		НоменклатураЕГАИСОбъект.Записать();
