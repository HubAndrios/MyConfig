						...
						Если ЭлектронныйДокумент.Свойство("ДополнительныеПараметры") И ЗначениеЗаполнено(ЭлектронныйДокумент.ДополнительныеПараметры)
							...
							Если ВходящаяВерсияФормата <> ТекущаяВерсияФормата
								...
								Если НЕ (ЭтоПрямойОбмен(ПараметрыПакетаЭД.СпособОбменаЭД)
										И (АктуальнаяВерсияФормата = НСтр("ru = 'ФНС 5.01 (УКД:Первичный документ)'")
											ИЛИ АктуальнаяВерсияФормата = НСтр("ru = 'ФНС 5.01 (УПД:Первичный документ)'"))) Тогда
...
//
// Параметры:
//  Cертификат - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - сертификат электронной подписи и шифрования.
//
// Возвращаемое значение:
...
Функция ЭтоСертификатОблачногоСервиса(Сертификат) Экспорт
	...
	Если ЗначениеЗаполнено(Отпечатки) Тогда
		...
		ОтпечатокСертификата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сертификат, "Отпечаток");
		Если Отпечатки.Получить(ОтпечатокСертификата) <> Неопределено Тогда
...
Функция ЭлектронныйДокументПолностьюПодписан(СсылкаНаФайлЭД)
	...
	Для Каждого Элемент Из УстановленныеПодписи Цикл
		МассивОтпечатковСертификатов.Добавить(Элемент.Отпечаток);
	КонецЦикла;
	МассивОтпечатковСертификатов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивОтпечатковСертификатов);
	
	Если СсылкаНаФайлЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Интеркампани Тогда
		ФлагПодписанПолностью = МассивОтпечатковСертификатов.Количество() > 1;
	Иначе
...
// Только для внутреннего использования
Функция ЗашифрованныйМаркер(СтруктураПараметровЗапросаМаркера, ИнтернетПоддержкаПользователейПодключена = Истина, ПоказыватьОшибки = Ложь) Экспорт
	
	Соединение = ОбменСКонтрагентамиВнутренний.ПолучитьСоединение(СтруктураПараметровЗапросаМаркера.СпособОбменаЭД);
	Возврат ОбменСКонтрагентамиВнутренний.ПолучитьМаркерОЭДО(СтруктураПараметровЗапросаМаркера, Соединение, Ложь, ИнтернетПоддержкаПользователейПодключена, ПоказыватьОшибки)
