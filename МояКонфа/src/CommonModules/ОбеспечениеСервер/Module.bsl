...
// Сворачивает строки в таблице обеспечения.
// Если, например, в документе установлен признак "Не отгружать частями" - необходимо свернуть строки с одинаковым вариантом обеспечения,
// так как разибение по датам не имеет смысл.
// 
// Параметры:
...
//  ЕстьДатаОтгрузки   - Булево                  - признак, определяющий наличие в элементах параметра "ТаблицаОбеспечения"
//                                                 данных о дате отгрузки. Если даты различны, строки не будут свернуты.
//  ЕстьСерия          - Булево                  - признак, определяющий наличие в элементах параметра "ТаблицаОбеспечения"
//                                                 данных о серии. Если серии различны, строки не будут свернуты.
//  Отказ              - Булево                  - признак необходимости сворачивания строк в таблице с одинаковым вариантом обеспечения,
//                                                 в пределах одного идентификатора исходной строки документа.
//
Процедура СвернутьСтрокиДляОтгрузкиОднойДатой(ТаблицаОбеспечения, ЖелаемаяДатаОтгрузки, ЕстьДатаОтгрузки, ЕстьСерия, Отказ) Экспорт
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЕстьДатаОтгрузки Тогда
		
		ДатаПоУмолчанию = Макс(НачалоДня(ТекущаяДатаСеанса()), ЖелаемаяДатаОтгрузки);
		Для Каждого СтрокаОбеспечения Из ТаблицаОбеспечения Цикл
			
			СдвинутьДату = СтрокаОбеспечения.Отгружено = 0 И СтрокаОбеспечения.ДатаОтгрузки < ДатаПоУмолчанию;
			
			Если СдвинутьДату Тогда
				СтрокаОбеспечения.ДатаОтгрузки = ДатаПоУмолчанию;
			КонецЕсли;
...
// Изменяет дату отгрузки в коллекции на желаемую дату отгрузки.
// 
// Параметры:
//   ДанныеЗаполнения     - ТаблицаЗначений - коллекция в которой нужно выполнить действие.
//   ЖелаемаяДатаОтгрузки - Дата            - желаемая дата отгрузки.
//   Отказ                - Булево          - признак отказа от выполнения действия.
// 
// Возвращаемое значение:
//   Булево - признак, что коллекция требует свертки (ввиду того, что могут совпадать варианты обеспечения,
//            для случая отгрузки одной датой, либо варианты обеспечения и даты отгрузки.
//
Функция СдвинутьДатуОтгрузкиКЖелаемой(ДанныеЗаполнения, ЖелаемаяДатаОтгрузки, Отказ) Экспорт
	
	Если Отказ Тогда
		// Если заказ в режиме "Отгружать одной датой" требуется свертка по вараинтам обеспечения независимо от исходных дат отгрузки.
		Возврат Истина;
	КонецЕсли;
	...
	Для Каждого СтрокаОбеспечения Из ДанныеЗаполнения Цикл
		...
		Если СдвинутьДату И Не ТребуетсяСвертка Тогда
