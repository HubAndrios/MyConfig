...
//  ИмяМодуля - Строка - уникальное название внешнего модуля.
//  ПроверятьВерсию - Булево - признак необходимости проверки версии компоненты.
//
...
Процедура ПослеПопыткиПодключенияВнешнейКомпоненты(Результат, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		КонецПопытки;
	Иначе
		Оповещение = Новый ОписаниеОповещения("ПодключитьВнешнююКомпонентуПослеУстановки", ЭтотОбъект, Параметры);
		НачатьУстановкуВнешнейКомпоненты(Оповещение, Параметры.АдресВК);
	КонецЕсли;
