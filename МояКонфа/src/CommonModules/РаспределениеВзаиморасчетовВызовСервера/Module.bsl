...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		Производительность.ЗафиксироватьЗамерВремени(ОписаниеЗамера, КоличествоПовторяющихсяДанных);
		КоличествоПовторяющихсяДанных = 0;
		
		// Этап 2Х: Переоценка расчетов на конец периода
		Документы.ПереоценкаВалютныхСредств.ПереоценитьРасчетыСКлиентами(СписокОрганизаций, КонецМесяца(НачалоПериода), ДанныеКРасчетуСКлиентамиЗаМесяц.АналитикиУчетаПоПартнерам, КоличествоПовторяющихсяДанных);
		Производительность.ЗафиксироватьЗамерВремени(ОписаниеЗамера, КоличествоПовторяющихсяДанных);
		КоличествоПовторяющихсяДанных = 0;
		...
		Производительность.ЗакончитьЗамерВремени(ОписаниеЗамера, КоличествоПовторяющихсяДанных);
		КоличествоПовторяющихсяДанных = 0;
	КонецЦикла;
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		// Этап 2Х: Переоценка расчетов на конец периода
		Документы.ПереоценкаВалютныхСредств.ПереоценитьРасчетыСПоставщиками(СписокОрганизаций, КонецМесяца(НачалоПериода), ДанныеКРасчетуСПоставщикамиЗаМесяц.АналитикиУчетаПоПартнерам, КоличествоПовторяющихсяДанных);
		Производительность.ЗафиксироватьЗамерВремени(ОписаниеЗамера, КоличествоПовторяющихсяДанных);
		КоличествоПовторяющихсяДанных = 0;
...
Функция ТекстДвиженияРасчетовСПоставщиками()
	Возврат "
		...
		|			И ТИПЗНАЧЕНИЯ(ДД.ЗаказПоставщику) НЕ В (&СписокТиповЗаказов)
...
Процедура ОчиститьЗаказыВДвиженияхРасчетовСПоставщиками() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Взаиморасчеты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Запуск задания очистки заказов в движениях расчетов с поставщиками.'"));
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Взаиморасчеты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Задание очистки заказов в движениях расчетов с поставщиками завершено.'"));
...
Процедура ОчиститьЗаказыВДвиженияхРасчетовСКлиентами() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Взаиморасчеты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Запуск задания очистки заказов в движениях расчетов с клиентами.'"));
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Взаиморасчеты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Задание очистки заказов в движениях расчетов с клиентами завершено.'"));
