...
Процедура ИнициализироватьОбщиеПараметрыРасчета(ПараметрыИнициализации, ПараметрыРасчета, ПараметрыОтладки = Неопределено) Экспорт
	...
	РешениеСЛУ.Вставить("ЗащитаОтПереполненияПоля",	   		   999999999.999999999); // защита от ошибки "Ошибка SQL: Переполнение поля", она же "Arithmetic overflow"
	...
	ПараметрыРасчета.ОписаниеПараметров.Вставить("РешениеСЛУ.ЗащитаОтПереполненияПоля",
		НСтр("ru='Максимально допустимое значение суммы в строке временной таблицы решений при решении СЛУ.
...
Функция ПараметрРасчетаМожноПереопределить(ИмяПараметра, ЗначениеПараметра) Экспорт
	...
	СлужебныеПараметры = ОбщегоНазначенияУТКлиентСервер.ПреобразоватьСтрокуКлючейВСоответствие(
		НРег("ЭтоПараметрыРасчетаПУ22, МестоВызоваРасчета, ЗапущенРасчетПартий, ИдетРасчетПартий, ИмяМодуляМеханизмаРасчета, ВариантРасчета,
		|ПредварительныйРасчет, РегламентноеЗадание, ПартионныйУчетВерсии22, ФормироватьНачальныеОстаткиПартий22,
		|УправленческийУчетОрганизаций, ИспользуетсяУправлениеВНА_2_4,
		|ЗаполненыПартииВСебестоимости, ЗаполняютсяПартииВСебестоимости, ЗаполняютсяПартииВВыручке, ЗаполненыПартииВВыручке,
		|ИзмененоДокументов, ОбщееКоличествоФЗ, СтрокаСоединенияИБ, ВыполняетсяОбновлениеИБ,
...
Функция СлужебныйСимвол_ПроблемаУжеЗарегистрирована() Экспорт
	Возврат Обработки.ОперацииЗакрытияМесяца.СлужебныйСимвол_ИсключениеУжеОтработаноВЭтапеРасчета()
КонецФункции
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		...
		НСтр("ru='Проверяется корректность движений по регистрам, сформированых при переходе на партионный учет версии 2.2:
...
Процедура ПроверкаКорректностиИсходныхДанныхДляРасчетаСебестоимости(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.СформироватьПараметрыРегистрацииПроблем(
	...
	ЗакрытиеМесяцаСервер.ДобавитьОписаниеРегистрацииПроблем(ПараметрыРегистрации,
		"ВТНеОформленоПоступлениеРабот",
		НСтр("ru = 'Не отражены поступления работ в подразделения организации ""%1"" за период %2'"),
		СписокПолей);
	
	ЗакрытиеМесяцаСервер.ЗарегистрироватьПроблемыПроверкиДоРасчетаПоДаннымВременнойТаблицы(
		ПараметрыПроверки,
...
Процедура ПроверкаКорректностиПереходаНаПартионныйУчетВерсии22(ПараметрыПроверки) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗарегистрироватьПроблемыПроверкиПослеРасчетаПоДаннымВременнойТаблицы(
		ПараметрыПроверки,
		ПараметрыРегистрации,
		ТекстЗапроса);
...
Процедура ПроверкаСоответствияСуммИКоличества(ПараметрыПроверки) Экспорт
	...
	|		И Т.Количество
	|			* (Т.СтоимостьБезНДС + Т.ДопРасходыБезНДС
...
Процедура ПроверкаЗатратНеРаспределенныхНаСебестоимость(ПараметрыПроверки) Экспорт
	...
	|					Т.Ссылка
	|				ИЗ
	|					ВТСтатьиРасходовРегл КАК Т)) КАК Т
	|";
...
Процедура ЗарегистрироватьПроблемуВыполненияРасчета(ПараметрыРасчета, МассивОрганизаций = Неопределено,
	...
	//Запишем информацию о наличии ошибок в механике закрытия месяца.
...
// Проверяет возможность изменения данных в указанных разделах учета.
//
// Параметры:
//	Дата - Дата - период, начиная с которого необходимо изменение данных
//	МассивРазделов - Массив или ПланВидовХарактеристик.РазделыДатЗапретаИзменения.Ссылка - проверяемые разделы учета
//	МассивОрганизаций - Массив или СправочникСсылка.Организации - проверяемые организации
//
// Возвращаемое значение:
//	Структура с ключами
//		ИзмененияЗапрещены - Булево - признак установленной даты запрета изменений
//		Пояснения - Строка - текст, описывающий установленные даты запрета
//
Функция ПроверитьДатуЗапретаИзмененияПоРазделамИОрганизациям(Дата, МассивРазделов, МассивОрганизаций) Экспорт
	
	// Подготовим данные для проверки.
	ТаблицаДанных = ДатыЗапретаИзменения.ШаблонДанныхДляПроверки();
	
	Для Каждого ТекущаяОрганизация Из ОбщегоНазначенияУТКлиентСервер.Массив(МассивОрганизаций) Цикл
		
		Для Каждого ТекущийРаздел Из ОбщегоНазначенияУТКлиентСервер.Массив(МассивРазделов) Цикл
	...
	ИзмененияЗапрещены = ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ТаблицаДанных, ОписаниеДанных, ТекстОшибки);
	
	// Обработаем результат проверки.
	СжатыйТекстОшибки = "";
	МассивСтрок = Новый Массив;
	
	Для НомерСтроки = 1 По СтрЧислоСтрок(ТекстОшибки) Цикл
		
		ТекущаяСтрокаОшибки = СокрЛП(СтрПолучитьСтроку(ТекстОшибки, НомерСтроки));
		
		Если ЗначениеЗаполнено(ТекущаяСтрокаОшибки) Тогда
			ОбщегоНазначенияУТКлиентСервер.ДобавитьНовоеЗначениеВМассив(МассивСтрок, ТекущаяСтрокаОшибки);
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого ТекущаяСтрокаОшибки Из МассивСтрок Цикл
		СжатыйТекстОшибки = СжатыйТекстОшибки + Символы.ПС + ТекущаяСтрокаОшибки;
	КонецЦикла;
	
	// Подготовим возвращаемый результат
	РезультатПроверки = Новый Структура;
	РезультатПроверки.Вставить("ИзмененияЗапрещены", ИзмененияЗапрещены);
	РезультатПроверки.Вставить("Пояснения", 		 СокрЛП(СжатыйТекстОшибки));
