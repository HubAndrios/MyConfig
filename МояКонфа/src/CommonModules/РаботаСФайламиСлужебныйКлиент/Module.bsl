...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Попытка
		Если РаботаСФайламиСлужебныйВызовСервера.ЭтоСправочникФайлы(ПараметрыВыполнения.ВладелецФайла) Тогда
...
Процедура ДобавитьСоСканера(ПараметрыВыполнения) Экспорт
	...
	ПараметрыФормы = Новый Структура("ВладелецФайла, ЭтоФайл, НеОткрыватьКарточкуПослеСозданияИзФайла");
...
Процедура ДобавитьНаОсновеШаблонаПослеВыбораШаблона(Результат, ПараметрыВыполнения) Экспорт
	...
	ОбработчикРезультата = ПодготовитьОбработчикДляДиалога(ПараметрыВыполнения.ОбработчикРезультата);
	ИмяСправочникаХранилищаФайлов = ?(ПараметрыВыполнения.Свойство("ИмяСправочникаХранилищаФайлов"),
	...
	Если ИмяСправочникаХранилищаФайлов = "Файлы" Тогда
