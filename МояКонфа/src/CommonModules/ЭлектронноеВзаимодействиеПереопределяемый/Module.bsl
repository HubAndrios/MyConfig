...
//  СтруктураПараметров - Структура - дополнительные параметры, содержит поля:
//     * ТекстСостоянияЭДО - Строка - текущее состояние электронного документа
//     * ВидОперации - Строка - вид выполняемой операции, содержит значение "УстановкаГиперссылки"
//     * ЗначениеПараметра - Булево - если Истина, то состояние должно отображаться в виде гиперссылки.
//
...
// Определяет названия функциональных опций в прикладном решении, по представлению БЭД
//
// Параметры:
//  СоответствиеФО - Соответствие - список функциональных опций. При вызове передается пустое соответствие.
//   Возможно добавление одной записи с ключом "ИспользоватьПартнеровИКонтрагентов".
//   Если в прикладном решении нет такой фукнциональной опции, то добалять ничего не нужно.
//
...
//           СоответствиеРеквизитовОбъекта.Вставить("ПриобретениеТоваровУслугВМетаданных", ...);
...
Процедура ПолучитьСоответствиеНаименованийОбъектовМДИРеквизитов(СоответствиеРеквизитовОбъекта) Экспорт
	...
	СоответствиеРеквизитовОбъекта.Вставить("СчетФактураВыданныйАвансВМетаданных",  	"СчетФактураВыданныйАванс");
	СоответствиеРеквизитовОбъекта.Вставить("СчетФактураПолученныйВМетаданных",     	"СчетФактураПолученный");
	СоответствиеРеквизитовОбъекта.Вставить("СчетФактураПолученныйАвансВМетаданных", "СчетФактураПолученныйАванс");
...
// Используется при выводе сообщений для пользователя. С полным списком представлений находится в общем макете ПрикладноеПредставлениеРеквизитов_ru.
// Если название поля не соответствует прикладному, то его нужно переопеределить, добавив в соответствие, например:
...
// Получает ключевые реквизиты объекта, данные которых используются при формировании электронного документа.
// При изменении в исходном объекте ключевых реквизитов, ЭДО становится неактуальным.
//
// Параметры:
...
//    Первым элементов является Шапка, в значении которого перечислены ключевые реквизиты через запятую.
//    Затем идут элементы с названиями табличных частей, значениями которых является строка с названиями ключевых реквизитов табличной части, перечисленные через запятую.
//    Пример заполнения:
...
//
// Параметры:
//  ВидОперации    - Строка - признак выполняемой операции, возможные значения:
//    "РАБОТАСБАНКАМИ", "РаботаСЭД", "НАСТРОЙКАКРИПТОГРАФИИ", "ПодписаниеЭД".
//  ТекстСообщения - Строка - (возвращаемое значение) текст сообщения.
//
...
// Полный перечень ошибок представлен в ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ИнициализацияСообщенийОшибок
//
// Параметры:
//  КодОшибки - Строка - код ошибки
//  ТекстОшибки - Строка - (возвращаемое значение) измененный текст ошибки.
//
...
//
// Параметры:
//  ТекстСообщения - Строка - текст сообщения, содержит значение НСтр("ru = 'Нарушение прав доступа.'");
//
...
// Находит ссылку на объект информационной базы
// 
// Параметры:
//  ТипОбъекта - Строка - идентификатор типа объекта, который необходимо найти;
//  ИДОбъекта - Строка - ключевой идентификатор объекта заданного типа для поиска;
...
//
// Возможные следующие значения параметров вызова функции
//
//   ЭлектронноеВзаимодействие:
//
//   ТипОбъекта = "БанковскиеСчетаОрганизаций"
//   ИдОбъекта = номер банковского счета
//   ДополнительныеРеквизиты.Владелец = ссылка на организацию (может отсутствовать)
//
//
//   ЭлектронноеВзаимодействие.ОбменСБанками:
//
...
//   ДополнительныеРеквизиты.ИНН
//   ДополнительныеРеквизиты.КПП
//
//   ТипОбъекта = "Банки"
//   ИдОбъекта = БИК
//   ДополнительныеРеквизиты.Код
//   ДополнительныеРеквизиты.Наименование
//
//   ЭлектронноеВзаимодействие.ОбменСКонтрагентами:
//
//   ТипОбъекта = "ЕдиницыИзмерения"
//   ИдОбъекта = код
//   ДополнительныеРеквизиты.Код (может отсутствовать)
//   ДополнительныеРеквизиты.Наименование (может отсутствовать)
//
//   ТипОбъекта = "ВидыКонтактнойИнформации"
//   ИдОбъекта = "EmailОрганизации"
//   ИдОбъекта = "ТелефонОрганизации"
//   ИдОбъекта = "ФаксОрганизации"
//
//   ТипОбъекта = "Номенклатура"
//   ИдОбъекта = идентификатор товара
//   ДополнительныеРеквизиты.Идентификатор
//   ДополнительныеРеквизиты.Артикул
//
//   ТипОбъекта = "НоменклатураПоставщиков"
//   ИдОбъекта = идентификатор товара
//   ДополнительныеРеквизиты.Владелец
//   ДополнительныеРеквизиты.Наименование
//   ДополнительныеРеквизиты.Идентификатор
//   ДополнительныеРеквизиты.ТоварКод
//   ДополнительныеРеквизиты.Артикул
//   ДополнительныеРеквизиты.НаименованиеХарактеристики
//
//   ТипОбъекта = "Валюты"
//   ИдОбъекта = код валюты
//
//   ТипОбъекта = "Контрагенты"
//   ИдОбъекта = идентификатор контрагента
//   ДополнительныеРеквизиты.ИНН
//   ДополнительныеРеквизиты.КПП
//   ДополнительныеРеквизиты.ПолноеНаименование
//   ДополнительныеРеквизиты.ЮридическийАдрес_Представление
//   ДополнительныеРеквизиты.АдресРегистрации_Представление
//
//   ТипОбъекта = "Партнеры" (может быть переопределен в ПолучитьСоответствиеСправочников)
//   ДополнительныеРеквизиты.Контрагент
//
//   ТипОбъекта = "БанковскиеСчетаКонтрагентов"
//   ИдОбъекта = номер банковского счета
//   ДополнительныеРеквизиты.Владелец = ссылка на организацию (может отсутствовать)
//
//   ТипОбъекта = "ДоговорыКонтрагентов"
//   ДополнительныеРеквизиты.Владелец
//   ДополнительныеРеквизиты.НомерДоговора
//   ДополнительныеРеквизиты.ДатаДоговора
//
...
//
// Возвращаемое значение:
//  Структура - данные юридического (физического лица). Содержит поля:
//     Представление, ПолноеНаименование, КодПоОКПО, ИНН, КПП, Телефоны, ЮридическийАдрес, Банк, БИК, КоррСчет, НомерСчета, Фамилия, Имя, Отчество.
//
...
//
// Параметры:
//  СведенияОрганизации - Структура - сведения об организации, по которой надо составить описание. Может содержать поля:
//   ПолноеНаименование,ИНН,КПП,КодПоОКПО,ЮридическийАдрес,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет
//  Список - Строка - список запрашиваемых параметров организации. Может содержать поля:
//   ПолноеНаименование,ИНН,КПП,КодПоОКПО,ЮридическийАдрес,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет
