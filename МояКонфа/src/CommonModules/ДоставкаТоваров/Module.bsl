	...
	Для Каждого Ссылка из МассивСсылок Цикл
		...
		Если ДоставкаТоваровКлиентСервер.ЭтоРаспоряжениеНаДоставкуНаНашСклад(Ссылка) Тогда
			...
			Если (ПараметрыПриемки.СоглашениеЯвляетсяРаспоряжением 
				...
				Или (Не ПараметрыПриемки.ЭтоДокументПоступление 
...
Процедура ПриЧтенииСозданииРаспоряженийНаСервере(ЭлементыФормы,
	...
	СоглашениеЯвляетсяРаспоряжением = Ложь; РаспоряжениеПоСоглашению = Ложь;
	...
	Если ЭтоДоставкаНаНашСклад И ДокОбъект.Свойство("Ссылка") И ДокОбъект.Свойство("ВариантПриемкиТоваров") Тогда
		ПараметрыПриемки = ПолучитьПараметрыПриемки(ДокОбъект, ДокОбъект.ВариантПриемкиТоваров);
	КонецЕсли;
	...
	Если ЭлементыФормы.Найти("СтраницыДоставки") <> Неопределено Тогда
		Если ЭтоДоставкаНаНашСклад Тогда
			...
			Если РаспоряжениеПоСоглашению И ДокОбъект.Свойство("Соглашение") И ЭлементыФормы.Найти("ГруппаСводнаяИнформацияПоДоставке") <> Неопределено Тогда
				...
				СпособДоставки = ?(РаспоряжениеПоСоглашению, СпособДоставки, ДокОбъект.СпособДоставки);
			КонецЕсли;
		КонецЕсли;
		ДоставкаТоваровКлиентСервер.УстановитьСтраницуДоставки(ЭлементыФормы, СпособДоставки, ИспользоватьЗаданияНаПеревозкуДляУчетаДоставкиПеревозчиками, РаспоряжениеПоСоглашению);
...
Процедура ПриИзмененииСпособаДоставки(ЭлементыФормы, ДокОбъект) Экспорт
	...
	Если ЭлементыФормы.Найти("СтраницыДоставки") <> Неопределено Тогда
		...
		ДоставкаТоваровКлиентСервер.УстановитьСтраницуДоставки(ЭлементыФормы, ДокОбъект.СпособДоставки, ПолучитьФункциональнуюОпцию("ИспользоватьЗаданияНаПеревозкуДляУчетаДоставкиПеревозчиками"), РаспоряжениеПоСоглашению);
	КонецЕсли;
...
Процедура ОбновитьСписокРаспоряженийНаДоставку(Форма, ОбновляемыеРаспоряжения = Неопределено) Экспорт
	...
	ТоварыКПоступлениюДвижения = Пакет[Пакет.ВГраница()-1].Выгрузить();
		...
		Пока Выборка.СледующийПоЗначениюПоля("Распоряжение") Цикл
			...
			Пока Выборка.СледующийПоЗначениюПоля("Склад") Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если ИсходящаяДоставка ИЛИ НЕ ЕстьОрдерныйНаПоступлениеСклад Тогда
					...
					Иначе
						...
						ПараметрыОтбора = Новый Структура("Распоряжение,Склад,Номенклатура,Характеристика,Назначение");
						ЗаполнитьЗначенияСвойств(ПараметрыОтбора, Выборка);
						ДетальныеДанные = ТоварыКПоступлениюДвижения.НайтиСтроки(ПараметрыОтбора);
						
						КоличествоКРаспределению = Выборка.Количество;
						ОсталосьРаспределить = КоличествоКРаспределению;
...
Процедура ЗаполнитьПризнакДоставляетсяПолностью(ТоварыРаспоряжений,
	...
	ТекстРаспоряженияНаДоставку = ТекстРаспоряженияНаДоставку + ТекстЗапросаПоСоглашениям;
	
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияУТ.РазделительЗапросовПакета() + ТекстРаспоряженияНаДоставку + ОбщегоНазначенияУТ.РазделительЗапросовПакета();
	...
	|ГДЕ
	|	ОстаткиПоРаспоряжению.Распоряжение ЕСТЬ NULL И НЕ ОстаткиПоРаспоряжению.Распоряжение ССЫЛКА Справочник.СоглашенияСПоставщиками" + ОбщегоНазначенияУТ.РазделительЗапросовПакета();
...
//
// Параметры:
//  Соглашение - Справочник.СоглашенияСПоставщиками.Ссылка - соглашение, содержащее информацию по реквизитам доставки.
// Возвращаемое значение:
//  Строка - текстовое представление реквизитов доставки.
Функция СформироватьСводнуюИнформациюПоДоставке(Соглашение) Экспорт
	...
	РеквизитыДоставки = РеквизитыДоставки(Соглашение);
	...
	Если ЗначениеЗаполнено(РеквизитыДоставки.ВремяДоставкиС) И ЗначениеЗаполнено(РеквизитыДоставки.ВремяДоставкиПо) Тогда
		...
		НСтр("ru='с '") 	+ Формат(РеквизитыДоставки.ВремяДоставкиС	, "ДФ='HH"":""mm'") +
		НСтр("ru=' до '") 	+ Формат(РеквизитыДоставки.ВремяДоставкиПо	, "ДФ='HH"":""mm'"); 
	...
	ИначеЕсли ЗначениеЗаполнено(РеквизитыДоставки.ВремяДоставкиС) Тогда
		ПредставлениеВремениДоставки = 
		НСтр("ru='после '") + Формат(РеквизитыДоставки.ВремяДоставкиС	, "ДФ='HH"":""mm'"); 
		РеквизитыДоставки.Вставить("ВремяДоставки", ПредставлениеВремениДоставки);	
...
Функция ТекстЗапросаЗаполненияЗаданияНаПеревозкуПоРасходнымОрдерам()
	...
	Для Каждого ТипРаспоряжениеНаДоставку Из Метаданные.ОпределяемыеТипы.РаспоряжениеНаДоставку.Тип.Типы() Цикл
		...
		Если ТипРаспоряжениеНаДоставку = Тип("СправочникСсылка.СоглашенияСПоставщиками") Тогда
			Продолжить;
		КонецЕсли;
...
Функция УстановитьВидимостьЭлементовДоставки(ЭлементыФормы, ДокОбъект, НакладнаяПоЗаказам, ПараметрыПриемки = Неопределено, ЭтоДоставкаНаНашСклад = Неопределено)
	...
	Если ИспользоватьУправлениеДоставкой Тогда
		Если ЭтоДоставкаНаНашСклад Тогда 
			ВидимостьДоставки = ПараметрыПриемки.СоглашениеЯвляетсяРаспоряжением 
...
Функция ПолучитьПараметрыПриемки(ОбъектСсылка, Знач ВариантПриемкиТоваров = Неопределено, НакладнаяПоЗаказам = Ложь)
	...
	Если ВариантПриемкиТоваров = Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
			...
			Если ЗначениеЗаполнено(ОбъектСсылка.Соглашение) Тогда
				ВариантПриемкиТоваров = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектСсылка.Соглашение, "ВариантПриемкиТоваров");
			Иначе
		...
		Иначе
			...
			|		ПО ПриобретениеТоваровУслуг.Соглашение = СоглашенияСПоставщиками.Ссылка
			|ГДЕ
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СоглашенияСПоставщиками КАК СоглашенияСПоставщиками
			|		ПО ЗаказПоставщику.Соглашение = СоглашенияСПоставщиками.Ссылка
			|ГДЕ
	...
	Если Справочники.СоглашенияСПоставщиками.СоглашениеИспользуетсяПриПриемке(ВариантПриемкиТоваров) Тогда
		...
		ИначеЕсли ВариантПриемкиТоваров = ПредопределенноеЗначение("Перечисление.ВариантыПриемкиТоваров.МожетПроисходитьБезЗаказовИНакладных") Тогда 
			...
			СтруктураВозврата.ДокументПоступление		= Тип("СправочникСсылка.СоглашенияСПоставщиками");
			СтруктураВозврата.РаспоряжениеНаПриемку		= Тип("ДокументСсылка.СоглашениеСПоставщиком");
...
Функция ТекстЗапросаРаспоряженияНаДоставкуПолучателямПоОрдернойСхеме(ВыборочноеОбновление)
	...
	|			ПО ТоварыКОтгрузке.ДокументОтгрузки = РаспоряженияНаДоставку.Распоряжение
	|			И ТоварыКОтгрузке.Склад = РаспоряженияНаДоставку.Склад
	|	
	|	ГДЕ
	...
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		МАКСИМУМ(НАЧАЛОПЕРИОДА(ТоварыКОтгрузке.Период, ДЕНЬ)) КАК ДатаОтгрузки,
	|		ВЫБОР КОГДА
	|			НАЧАЛОПЕРИОДА(ТоварыКОтгрузке.Период, ДЕНЬ) < НАЧАЛОПЕРИОДА(&ТекущаяДатаСеанса, ДЕНЬ)
	|				ТОГДА НАЧАЛОПЕРИОДА(&ТекущаяДатаСеанса, ДЕНЬ)
	|			ИНАЧЕ НАЧАЛОПЕРИОДА(ТоварыКОтгрузке.Период, ДЕНЬ)
	...
	|		ТоварыСУчетомДоставленных.Зона КАК Зона,
	|		ТоварыСУчетомДоставленных.ПолучательОтправитель КАК ПолучательОтправитель,
	|		ТоварыСУчетомДоставленных.СпособДоставки КАК СпособДоставки,
	|		ТоварыСУчетомДоставленных.Перевозчик КАК Перевозчик,
	...
	|			ПО ТоварыСУчетомДоставленных.Распоряжение = ТоварыКОтгрузке.ДокументОтгрузки
	|				И ТоварыСУчетомДоставленных.Номенклатура = ТоварыКОтгрузке.Номенклатура
	|				И ТоварыСУчетомДоставленных.Характеристика = ТоварыКОтгрузке.Характеристика
	|				И ТоварыСУчетомДоставленных.Назначение = ТоварыКОтгрузке.Назначение
	|				И ТоварыСУчетомДоставленных.Серия = ТоварыКОтгрузке.Серия
	|				И ТоварыСУчетомДоставленных.Склад = ТоварыКОтгрузке.Склад
	|	ГДЕ
	...
	|			ИНАЧЕ НАЧАЛОПЕРИОДА(ТоварыКОтгрузке.Период, ДЕНЬ)
	|		КОНЕЦ,
	|		ТоварыСУчетомДоставленных.Склад,
	|		ТоварыСУчетомДоставленных.Распоряжение,
	|		ТоварыСУчетомДоставленных.Зона,
	|		ТоварыСУчетомДоставленных.ПолучательОтправитель,
	|		ТоварыСУчетомДоставленных.СпособДоставки,
	|		ТоварыСУчетомДоставленных.Перевозчик,
	...
	|		ТоварыСУчетомДоставленных.Количество,
	|		ТоварыСУчетомДоставленных.Количество * &ТекстЗапросаВесУпаковки,
	|		ТоварыСУчетомДоставленных.Количество * &ТекстЗапросаОбъемУпаковки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Функция ТекстЗапросаРаспоряженияНаДоставкуПолучателямБезОрдернойСхемы(ВыборочноеОбновление)
	...
	|	ЧастичноДоставленныеЗаказы.Серия КАК Серия,
	|	ЧастичноДоставленныеЗаказы.Количество КАК Количество,
	|	НАЧАЛОПЕРИОДА(РаспоряженияНаДоставку.Распоряжение.ДатаОтгрузки, ДЕНЬ) КАК Дата,
	|	РаспоряженияНаДоставку.СпособДоставки,
	|	РаспоряженияНаДоставку.ОсобыеУсловияПеревозки
...
Функция ТекстЗапросаРаспоряженияНаДоставкуОтОтправителейПоОрдернойСхеме(ВыборочноеОбновление)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РаспоряженияНаДоставку КАК РаспоряженияНаДоставку
	|		ПО ТоварыКПоступлениюОстатки.Склад = РаспоряженияНаДоставку.Склад
	|			И ТоварыКПоступлениюОстатки.ДокументПоступления = РаспоряженияНаДоставку.Распоряжение
	|ГДЕ
	|	ТоварыКПоступлениюОстатки.КПоступлениюОстаток > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ТоварыКПоступлению.КПоступлению, 0) КАК Количество,
	|	ЕСТЬNULL(ТоварыКПоступлению.КПоступлению, 0) * &ТекстЗапросаВесУпаковки КАК Вес,
	|	ЕСТЬNULL(ТоварыКПоступлению.КПоступлению, 0) * &ТекстЗапросаОбъемУпаковки КАК Объем
	|ИЗ
	|	ТоварыСУчетомДоставленных КАК ТоварыСУчетомДоставленных
...
Функция ТекстЗапросаРаспоряженияНаДоставкуОтОтправителейБезОрдернойСхемы(ВыборочноеОбновление)
	...
	|		ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) КАК Серия,
	|		ТоварыКДоставкеПоРегистраторам.КПоступлениюПриход КАК НачальнаяПотребностьПоДатам,
	|		ТоварыКДоставкеПоРегистраторам.КПоступлениюПриход * &ТекстЗапросаВесУпаковки КАК НачальнаяПотребностьПоДатамВес,
	|		ТоварыКДоставкеПоРегистраторам.КПоступлениюПриход * &ТекстЗапросаОбъемУпаковки КАК НачальнаяПотребностьПоДатамОбъем,
	...
	|	ИЗ
	|		РегистрНакопления.ТоварыКПоступлению.Обороты(, , Регистратор, ) КАК ТоварыКДоставкеПоРегистраторам
	...
	|	ГДЕ
	|		ТоварыКДоставкеПоРегистраторам.КПоступлениюПриход > 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
