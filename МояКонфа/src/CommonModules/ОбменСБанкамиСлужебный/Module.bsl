...
Функция ТекстЗапросаСостоянияДокументовСбербанк(НастройкаОбмена, ИдентификаторыДокументов) Экспорт
	...
	Попытка
		...
		Для Каждого ИдентификаторДокумента Из ИдентификаторыДокументов Цикл
			...
			ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(DocId, "docid", ИдентификаторДокумента, Истина, ТекстОшибки);
	...
	Исключение
		...
		Операция = НСтр("ru = 'Формирование запроса статусов электронных документов.'");
...
Функция БанкиДляНастройкиОбменаЧерезДиректБанк(ПараметрыОтбора = Неопределено, Проект = "1") Экспорт
	...
	Для Индекс = 2 По КоличествоЗаписей Цикл
		...
		Если СтрНайти(ПроектыСтрокой, Проект) Тогда
			...
			НайденнаяСтрока = БанкиБезНастройкиОбмена.Найти(БИКБанка, "БИК");
...
Функция СохранитьСообщениеОбмена(Знач ПараметрыСообщения, НовоеСообщениеСсылка) Экспорт
	...
	Если ПараметрыСообщения.Свойство("ВнешнийИдентификатор")
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстСообщения, ТекстСообщения, "ОбменСБанками", Выборка.Ссылка);
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтправитьЗапросСостоянияЭДВБанк(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ДанныеАвторизации = Новый Структура("МаркерРасшифрованный", СтруктураПараметров.ИдентификаторСессии);
...
Процедура ПолучитьВыпискуБанкаАсинхронно(СтруктураПараметров, АдресХранилища) Экспорт
	...
	СтруктураПараметров.Вставить("МаркерРасшифрованный", СтруктураПараметров.ИдентификаторСессии);
			...
			Для Каждого ГруппаПодписи Из ЭД.Data.CryptoParameters.CustomerSignature.GroupSignatures Цикл
				Для Каждого Сертификат Из ГруппаПодписи.Certificate Цикл
					...
					Если ДанныеСертификата = Неопределено Тогда
						Возврат Неопределено;
					КонецЕсли;
					Отпечаток = ОтпечатокСертификата(ДанныеСертификата);
					Если Отпечаток = Неопределено Тогда
						Возврат Неопределено;
					Иначе
						ДанныеСертификатов.Вставить(Отпечаток, ДанныеСертификата);
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
...
Процедура СохранитьШтампыБанка(ВыпискаБанка)
	...
	СтруктураРазбораФайла = ОбменСБанкамиСлужебный.СформироватьДеревоРазбора(ДвоичныеДанныеЭД);
...
Функция ОтпечатокСертификата(ДанныеСертификата)
	...
	ДанныеСертификата = ДанныеСертификатаВФорматеDER(ДанныеСертификата);
...
Функция СформироватьПлатежноеТребованиеAsync(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	...
	Исключение
		...
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, "ОбменСБанками", СсылкаНаДокумент);
		ФайлСформирован = Ложь;
	КонецПопытки;
...
Функция СформироватьПлатежноеПоручение(СсылкаНаОбъект, НастройкиОбменаЭД)
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ВидОперации = НСтр("ru = 'Формирование ЭД'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстСообщения, , "ОбменСБанками", СсылкаНаОбъект);
	КонецЕсли;
...
Функция СформироватьПлатежноеТребование(СсылкаНаОбъект, НастройкиОбменаЭД)
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Операция = НСтр("ru = 'Формирование ЭД'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстСообщения, , "ОбменСБанками", СсылкаНаОбъект);
	КонецЕсли;
...
Процедура ПрочитатьВыпискуСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	ДебетВНациональнойВалюте = ?(ЭД.debetSumNat = Неопределено, ЭД.debetSumNat, Число(ЭД.debetSumNat));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
		НовыйЭД, "ДебетВНациональнойВалюте", ДебетВНациональнойВалюте);
	КредитВНациональнойВалюте = ?(ЭД.creditSumNat = Неопределено, ЭД.creditSumNat, Число(ЭД.creditSumNat));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
