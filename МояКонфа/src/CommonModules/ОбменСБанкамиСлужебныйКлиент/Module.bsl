...
Процедура УстановитьСоединениеИАутентифицироватьсяНаСервереСбербанк(Оповещение, НастройкаОбмена, ЭтоТест = Ложь) Экспорт
	...
	АутентифицироватьсяНаТокенеСбербанка(ОповещениеПослеАутентификацииНаАппаратномУстройстве, НастройкаОбмена, , ЭтоТест);
...
//
Процедура УстановитьКаналНаТокенеСбербанк(Оповещение, НастройкаОбмена = Неопределено) Экспорт
	
	НазваниеБизнесСистемы = ОбменСБанкамиСлужебныйВызовСервера.НазваниеБизнесСистемыСбербанк(НастройкаОбмена);
	ПодключаемыйМодуль = ДополнительныеВнешниеКомпонентыКлиент.ВнешняяКомпонентаИзКэша("VPNKeyTLS");
...
Процедура ОбработатьОшибкуПолученияСтатусаЗапросаСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
		ВидОперации, ПодробнаяИнформация, ТекстСообщения, "ОбменСБанками");
...
Процедура ОтправитьЗапросНовыхДокументовСбербанк(Оповещение, НастройкаОбмена, СертификатПодписи, ИдентификаторСертификата) Экспорт
	...
	РеквизитыНастройкиОбмена = Новый Структура("ИдентификаторОрганизации");
	...
	ПодписатьДанныеСбербанк(ОписаниеОповещения, СтрокаПодписиBase64, ИдентификаторСертификата, Представление);
...
//  ДвоичныеДанныеСертификата - ДвоичныеДанные - данные сертификата аутентификации
//  ИдентификаторСертификата - Строка - идентификатор сертификата на токене
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//
Процедура АутентифицироватьсяПоСертификатуСбербанк(Оповещение, НастройкаОбмена = Неопределено) Экспорт
	...
	ПолучитьСертификатСТокенаСбербанк(ОповещениеПослеУстановкиКанала);
...
//    ЭтоТест - Булево - если Истина, то будет недоступно сохранение пароля.
//
Процедура АутентифицироватьсяНаТокенеСбербанка(ОповещениеОВыполнении, НастройкаОбмена = Неопределено, ПринудительнаяАутентификация = Ложь, ЭтоТест = Ложь) Экспорт
	...
	ДополнительныеВнешниеКомпонентыКлиент.ПодключитьВнешнююКомпонентуПоИдентификатору(
		ОповещениеПослеПодключенияВК, "VPNKeyTLS", Ложь);
...
Процедура ПодписатьЭДСбербанк(Оповещение, НастройкаОбмена, МассивСообщенийОбмена) Экспорт
	...
	АутентифицироватьсяНаТокенеСбербанка(ОповещениеПослеАутентификацииНаТокене, НастройкаОбмена, ПринудительнаяАутентификация);
...
//
Процедура ПроверитьПодписьНаТокенеСбербанк(Оповещение, ПодписанныеДанные, Подпись, Сертификат) Экспорт
...
//
Процедура ПодписатьДанныеСбербанк(ОписаниеОповещения, СтрокаПодписи, ИдентификаторСертификата, Представление) Экспорт
...
Процедура ОтправитьЧерезДополнительнуюОбработку(Результат, Параметры) Экспорт
	...
	Иначе
		Для Каждого КлючИЗначение Из ДанныеДляОтправки Цикл
			...
			РеквизитыНастройкиОбмена = Новый Структура("ИмяВнешнегоМодуля");
			ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(КлючИЗначение.Ключ, РеквизитыНастройкиОбмена);
...
Процедура ПродолжитьПолучениеВыпискиПослеВводаPINКодаЧерезДополнительнуюОбработку(PINКод, Параметры) Экспорт
	...
	ПарольУстановлен = УстановитьПарольСертификатаЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль,
		ДанныеСертификата.ДвоичныеДанныеСертификата, ПарольПользователя, Параметры.ВыбранныйСертификат);
	Если НЕ ПарольУстановлен Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПродолжитьТестНастройкиОбменаПослеВводаПароляКСертификатуЧерезДополнительнуюОбработку(ПараметрВозврата, Параметры) Экспорт
	...
	Для Каждого КлючИЗначение Из Параметры.СоотвСертификатовИИхСтруктур Цикл
		...
		ПараметрыСертификата.Вставить("ПарольПользователя", ПараметрВозврата.ПарольПользователя);
		Прервать;
	КонецЦикла;
	...
	Параметры.Вставить("ПарольПользователя", ПараметрВозврата.ПарольПользователя);
	Параметры.Вставить("ДанныеСертификата", ДанныеСертификата);
...
Процедура ПровестиТестНастройки(Обработчик, НастройкаОбмена) Экспорт
	...
	ИначеЕсли ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
		ПровестиТестНастройкиСбербанк(Обработчик, НастройкаОбмена);
...
Процедура ПослеПолученияИнформацииОбОшибкеВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
		ДополнительныеПараметры.ВидОперации, ПодробныйТекстОшибки, , "ОбменСБанками");
...
//   * ДополнительныеПараметры - значение, которое было указано при создании объекта ОписаниеОповещения.
//
Процедура ПроверитьВерсиюВКСбербанк(Оповещение)
	
	ИнформацияОВК = ДополнительныеВнешниеКомпонентыВызовСервера.ПараметрыВнешнейКомпоненты("VPNKeyTLS");
...
Процедура ПослеПопыткиПодключенияВнешнейКомпоненты1СДляСбербанка(Результат, Параметры) Экспорт
	...
	Иначе
		Оповещение = Новый ОписаниеОповещения(
			"ПодключитьВнешнююКомпоненту1СДляСбербанкаПослеУстановки", ЭтотОбъект, Параметры);
...
Процедура УстановитьВиртуальныйКаналПослеАутентификацииНаАппаратномУстройствеСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	УстановитьКаналНаТокенеСбербанк(Оповещение, ДополнительныеПараметры.НастройкаОбмена);
...
Процедура ПровестиАутентификациюПослеПолученияФродПараметровСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Результат.Вставить("НомерКонтейнера", ЗначениеИзКэшаСбербанк("НомерКонтейнера"));
	
	СтрокаФрод = ОбменСБанкамиСлужебныйВызовСервера.Фрод(Результат);
	
	ЗакэшироватьПараметрСбербанка("ФродПараметры", Результат);
...
Процедура ПослеАутентификацииНаСервереСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстОшибки, ТекстОшибки, "ОбменСБанками", ДополнительныеПараметры.НастройкаОбмена);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаСервере, Ложь);
		Возврат;
	КонецЕсли;
	...
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаСервере, Истина);
...
//
Процедура ПолучитьСертификатСТокенаСбербанк(Оповещение)
...
Процедура ПолучитьПараметрыФродМониторингаПослеПодписиСолиСбербанк(Результат, Параметры) Экспорт
	...
	Если Не Результат.Успех Тогда
		ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеАутентификацииНаСервере, Ложь);
		Возврат;
	КонецЕсли;
	...
	ПолучитьПараметрыФродМониторингаСбербанк(Оповещение);
...
Процедура ПолучитьПараметрыПКПослеПолученияАдресаКомпьютераСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ПодключаемыйМодуль.НачатьВызовПолучитьПараметрыПК(
...
Процедура ПолучитьПараметрыDevicePrintПослеПолученияПараметровПК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ПодключаемыйМодуль.НачатьВызовПолучитьПараметрыDevicePrint(Оповещение, РазрядностьОС, РазрядностьПроцессора,
		ГлубинаЦвета, ШиринаМонитора, ВысотаМонитора, ПлотностьПикселяX, ПлотностьПикселяY, ШиринаРабочегоСтола,
		СмещениеЧасовогоПояса, УстановленныеКомпоненты, Онлайн, СглаживаниеШрифтов, ЯзыкСистемы, ЯзыкПользователя);
...
Процедура ОтправитьТестовыйЗапросПослеУстановкиСоединенияСбербанк(Результат, Параметры) Экспорт
	...
	ВыполнитьОтправкуДанныхСбербанк(Оповещение, Параметры.ТестоваяСтрока, Параметры.НастройкаОбмена);
...
Процедура ОтправитьЗапросНовыхДокументовПослеПодписиСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстЗапроса = ОбменСБанкамиСлужебныйВызовСервера.ТекстЗапросаНочнойВыписки(ДополнительныеПараметры.НастройкаОбмена,
		ДополнительныеПараметры.ИдентификаторЗапроса, ДополнительныеПараметры.ИдентификаторОрганизации, Результат.ЭП,
		ДополнительныеПараметры.СертификатПодписи);
	...
	ВыполнитьОтправкуДанныхСбербанк(Оповещение, ТекстЗапроса, ДополнительныеПараметры.НастройкаОбмена);
...
//    НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена с банком.
//
Процедура ВыполнитьОтправкуДанныхСбербанк(Оповещение, Данные, НастройкаОбмена = Неопределено) Экспорт
...
Процедура ПослеОтправкиЗапросовСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Ответ, 1, 23) = "00000000-0000-0000-0000" Тогда
		ТекстСообщения = ТекстСообщенияСбербанк(Ответ, "sendRequestsSRP");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиЗапросов);
		Возврат;
	Иначе
...
Процедура ОбработатьОшибкуОтправкиЗапросовСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
		ВидОперации, ПодробнаяИнформация, ТекстОшибки, "ОбменСБанками");
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиЗапросов);
...
Процедура ПослеПредварительнойАутентификацииПоСертификатуСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если КодВозврата <> "AA==" Тогда
		...
		Описание = НСтр("ru = 'Счет заблокирован.'");
	...
	Параметры.Вставить("ОповещениеПослеАутентификацииНаСервере",
		ДополнительныеПараметры.ОповещениеПослеАутентификацииПоСертификатуНаСервереСбербанк);
	Параметры.Вставить("НастройкаОбмена", ДополнительныеПараметры.НастройкаОбмена);
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПолучитьПараметрыФродМониторингаПослеПодписиСолиСбербанк", ЭтотОбъект, Параметры);
	
	Представление = НСтр("ru = 'Данные аутентификации'");
	
	ПодписатьДанныеСбербанк(
		ОписаниеОповещения, Соль, ДополнительныеПараметры.ДанныеСертификата.ИдентификаторСертификата, Представление);
...
Процедура ПослеПолученияТекстаОшибкиСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ДополнительныеПараметры.ВидОперации, ТекстОшибки, 
		ТекстОшибки, "ОбменСБанками", ДополнительныеПараметры.НастройкаОбмена);
...
Процедура АутентификацияНаСервереПослеУстановкиКаналаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	АутентифицироватьсяПоСертификатуСбербанк(
		ДополнительныеПараметры.ОповещениеПослеАутентификацииНаСервере, ДополнительныеПараметры.НастройкаОбмена);
...
Функция ТекстСообщенияСбербанк(Тикет, Метод)
	
	Если Метод = "sendRequestsSRP" Тогда
		...
		ИначеЕсли Тикет = "00000000-0000-0000-0000-000000000006" Тогда
			ТекстСообщения = НСтр("ru = 'В сообщении не найден orgId.
										|Обратитесь в техническую поддержку.'");
		...
		ИначеЕсли Тикет = "00000000-0000-0000-0000-000000000009" Тогда
			ТекстСообщения = НСтр("ru = 'Не найден пользователь в БД роутера.
										|Обратитесь в техническую поддержку.'");
		ИначеЕсли Тикет = "00000000-0000-0000-0000-000000000010" Тогда
			ТекстСообщения = НСтр("ru = 'Не найдена сессия.
										|Обратитесь в техническую поддержку.'");
		ИначеЕсли Тикет = "00000000-0000-0000-0000-000000000011" Тогда
			ТекстСообщения = НСтр("ru = 'Запрос размеров списка превышает максимальный.
										|Обратитесь в техническую поддержку.'");
...
Процедура ПроверитьУстановкуПодписиПолученияСертификатовСТокенаСбербанк(Результат, Параметры) Экспорт
	...
	Для Каждого КлючЗначение Из СоответствиеСертификатов Цикл
		...
		Если СтруктураСертификата.Отпечаток = РеквизитыПроверяемогоСертификата.Отпечаток Тогда
			...
			ПодписатьДанныеСбербанк(ОписаниеОповещения, СтрокаПодписиBase64, КлючЗначение.Ключ, Представление);
			Возврат
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодобратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ТекстОшибки, ТекстСообщения, "ОбменСБанками", ДополнительныеПараметры.НастройкаОбмена);
			Продолжить;
		КонецПопытки;
...
Процедура ОпределитьСтатусСледующейПодписиСбербанк(МассивСообщенийОбмена, СообщениеОбмена, ПодписанныеДанные, СоответствиеПодписейИСертификатов, РезультатПроверки, Параметры)
	...
	ПроверитьПодписьНаТокенеСбербанк(Оповещение, ПодписанныеДанные, Подпись, СтрокаСертификат);
...
Процедура ПослеПроверкиВерсииВКСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	УстановитьСоединениеИАутентифицироватьсяНаСервереСбербанк(
		Оповещение, ДополнительныеПараметры.НастройкаОбмена, Истина);
...
Процедура ПодписатьТестовыйЗапросПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	ПодписатьДанныеСбербанк(ОписаниеОповещения, СтрокаПодписиBase64, Результат.ИдентификаторСертификата, Представление);
...
Процедура ПроверитьПодписьПослеУстановкиСбербанк(Результат, Параметры) Экспорт
	...
	ИначеЕсли Результат.Успех Тогда
		...
		ПроверитьПодписьНаТокенеСбербанк(
			Оповещение, Параметры.СтрокаПодписиBase64, Результат.ЭП, ПараметрыСертификата.СертификатBase64);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Результат.ТекстОшибки);
		Результат = Новый Структура("Успех", Ложь);
		ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеТестаНастройки, Результат);
...
Процедура ОтправитьОчереднойПлатежныйДокументСбербанк(Параметры)
	...
	ВыполнитьОтправкуДанныхСбербанк(Оповещение, ПакетXML, Параметры.НастройкаОбмена);
...
Процедура ОбработатьРезультатПолученияДокументаСбербанк(Результат, Параметры) Экспорт
	...
	Если Результат = "<!--NOT PROCESSED YET-->" ИЛИ Результат = "<!--NOT_PROCESSED_YET-->" Тогда // Запрос еще не был обработан
		...
		Возврат
	ИначеЕсли Результат = "<!--REQUEST NOT FOUND-->" ИЛИ Результат = "<!--REQUEST_NOT_FOUND-->" Тогда // Идентификатор не найден в базе банка
		ОбменСБанкамиСлужебныйВызовСервера.УдалитьИдентификаторЗапроса(НастройкаОбмена, Параметры.Тикет);
		ПолучитьОчереднойДокументИзСбербанка(
			Параметры.НастройкаОбмена, Параметры.ИдентификаторОрганизации, Параметры.МассивТикетов, Параметры.Контекст);
		Возврат
	...
	ОбменСБанкамиСлужебныйВызовСервера.ОбработатьОтветСбербанка(
		Результат, НастройкаОбмена, МассивНовыхСообщенийОбмена, Параметры.Тикет, ЕстьОшибка);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
...
Процедура ПослеПолученияКонфигурационнойСтроки(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьОбработкуОповещения(
		ДополнительныеПараметры.ОповещениеПослеПолученияФродПараметров, ДополнительныеПараметры.ПараметрыФродМониторинга);
...
Процедура ПослеПолученияПараметровDevicePrint(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ПодключаемыйМодуль.НачатьВызовПолучитьКонфигурационнуюСтроку(Оповещение, КонфигурационнаяСтрока);
...
Процедура ПодписатьОчереднойЭДСбербанк(Параметры)
	...
	ПодписатьДанныеСбербанк(
		ОписаниеОповещения, СтрокаПодписиBase64, Параметры.ИдентификаторСертификата, ПодписываемоеСообщение);
...
Процедура ЗарегистрироватьсяНаТокенеПослеПолученияИнформацииОТокенеСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоТест ИЛИ ПринудительнаяАутентификация
		...
		Если Результат.ТипУстройства <> "SCR" Тогда // не экранный токен
			...
			ПараметрыФормы = Новый Структура("ЭтоТест", ДополнительныеПараметры.ЭтоТест);
			ОткрытьФорму("Обработка.ОбменСБанками.Форма.ЗапросПинКодаСбербанк", ПараметрыФормы, , , , , Обработчик);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьПараметрыФродМониторингаСбербанк(Оповещение)
...
Процедура ПровестиТестНастройкиСбербанк(Оповещение, НастройкаОбмена)
	...
	ОчиститьДанныеАвторизацииСбербанк();
	ДополнительныеВнешниеКомпонентыКлиент.УдалитьВнешнююКомпонентуИзКэш("VPNKeyTLS");
	...
	ПроверитьВерсиюВКСбербанк(ОписаниеОбработчика);
...
Процедура ОбработатьОшибкуВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, "ОбменСБанками");
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеПолученияСертификатов, Неопределено);
...
Процедура ПродолжитьПодписаниеЧерезДополнительнуюОбработку(Параметры)
	...
	ПарольУстановлен = УстановитьПарольСертификатаЧерезДополнительнуюОбработку(
		ВнешнийПодключаемыйМодуль, СертификатXML, СтруктураСертификата.ПарольПользователя, Параметры.ВыбранныйСертификат);
...
Процедура ПослеОпределенияСтатусаОчереднойПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстСообщения, ТекстСообщения, "ОбменСБанками", ДополнительныеПараметры.СообщениеОбмена);
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры)
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, , "ОбменСБанками", НастройкаОбмена);
		ОбменСБанкамиСлужебныйКлиент.УдалитьПарольИзСеанса(Параметры.ВыбранныйСертификат);
		ВыполнитьОбработкуОповещения(Параметры.ОбработчикПослеТестаНастройки, ТекстСообщения);
		Возврат;
	КонецПопытки;
...
Процедура ПродолжитьОтправкуПлатежныхДокументовПоНастройкамиПослеПолученияНовыхДокументовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат.Результат) = Тип("Строка") Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, Результат.Результат, Результат.Результат, "ОбменСБанками", ДополнительныеПараметры.НастройкаОбмена);
	КонецЕсли;
...
Процедура ПослеОтправкиПакетаЧерезВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстИсключения, , "ОбменСБанками", ДополнительныеПараметры.ПакетЭД);
