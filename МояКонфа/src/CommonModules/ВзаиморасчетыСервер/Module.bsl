...
Функция ПолучитьТекстЗапросаДокументыРасчетовСКлиентами(ВыборОснованияПлатежа, ПараметрыОтбора) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов) Тогда
		...
		Если ПараметрыОтбора.ОтборПоОрганизацииИКонтрагенту Тогда
			...
			|	ДанныеРегистра.ЗаказКлиента.ИдентификаторПлатежа КАК ИдентификаторПлатежа,
			|	ДанныеРегистра.ЗаказКлиента.Дата КАК Дата,
			|	ДанныеРегистра.ЗаказКлиента.Номер КАК Номер,
			|	"""" КАК НомерПоДаннымКонтрагента,
			|	ДАТАВРЕМЯ(1,1,1) КАК ДатаПоДаннымКонтрагента,
			|	ДанныеРегистра.ЗаказКлиента.Организация КАК Организация,
			|	ДанныеРегистра.ЗаказКлиента.Партнер КАК Партнер,
			|	ДанныеРегистра.ЗаказКлиента.Контрагент КАК Контрагент,
			|	ДанныеРегистра.ЗаказКлиента КАК Договор,
			|
			|	0 КАК Сумма,
			|	ДанныеРегистра.ЗаказКлиента.ВалютаВзаиморасчетов КАК Валюта,
			...
			|ГДЕ
			|	ДанныеРегистра.ЗаказКлиента.ПорядокРасчетов <> ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
			|	И ДанныеРегистра.ЗаказКлиента.ХозяйственнаяОперация В (
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту),
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию),
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию)
			|	)
...
Функция ПолучитьТекстЗапросаДокументыРасчетовСПоставщиками(ПараметрыОтбора) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов) Тогда
		...
		Если ПараметрыОтбора.ОтборПоОрганизацииИКонтрагенту Тогда
			...
			|	ДанныеРегистра.ЗаказПоставщику.Дата КАК Дата,
			|	ДанныеРегистра.ЗаказПоставщику.Номер КАК Номер,
			|	"""" КАК НомерПоДаннымКонтрагента,
			|	ДАТАВРЕМЯ(1,1,1) КАК ДатаПоДаннымКонтрагента,
			|	ДанныеРегистра.ЗаказПоставщику.Организация КАК Организация,
			|	ДанныеРегистра.ЗаказПоставщику.Партнер КАК Партнер,
			|	ДанныеРегистра.ЗаказПоставщику.Контрагент КАК Контрагент,
			|	ДанныеРегистра.ЗаказПоставщику.Ссылка КАК Договор,
			|	ДанныеРегистра.ЗаказПоставщику.ПлатежиПо275ФЗ КАК ПлатежиПо275ФЗ,
			|
			|	0 КАК Сумма,
			|	ДанныеРегистра.ЗаказПоставщику.ВалютаВзаиморасчетов КАК Валюта,
			|	ТИПЗНАЧЕНИЯ(ДанныеРегистра.ЗаказПоставщику) КАК Тип,
			|	Ложь КАК ЭтоРасчетыСКлиентами,
			|	ДанныеРегистра.ЗаказПоставщику КАК ОбъектРасчетов,
			|	
			|	ВЫБОР КОГДА ДанныеРегистра.ЗаказПоставщику.ПометкаУдаления ТОГДА
			|		1
			|	ИНАЧЕ
			|		0
			|	КОНЕЦ КАК СостояниеДокумента
			...
			|ГДЕ
			|	ДанныеРегистра.ЗаказПоставщику.ПорядокРасчетов <> ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
			|	И ДанныеРегистра.ЗаказПоставщику.ХозяйственнаяОперация В (
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщика),
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпорту),
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС),
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию),
...
//
// Возвращаемое значение:
// ТаблицаЗначений - содержит список платежных документов и дату выписки.
//
Функция ВыпискиПоРасчетномуСчету(НачалоПериода, КонецПериода, Организация, РасчетныйДокумент) Экспорт
	...
	|	И (ДанныеДокумента.Ссылка = &РасчетныйДокумент ИЛИ &РасчетныйДокумент = НЕОПРЕДЕЛЕНО)
	|	И (ДанныеДокумента.Организация В (&Организации) ИЛИ &ВсеОрганизации)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ХозяйственнаяОперацияСПоставщиком(ХозяйственнаяОперация) Экспорт
	...
	Если ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОплатаПоставщику") 
		ИЛИ ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура ДополнитьТекстЗапросаДокументыРасчетовСКлиентами(ТекстЗапроса, ТипОбъекта, ПараметрыОтбора)
	...
	Если ПараметрыОтбора.ЭтоУИП Тогда
		...
		Если ТипОбъекта <> Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
			...
			И ТипОбъекта <> Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
