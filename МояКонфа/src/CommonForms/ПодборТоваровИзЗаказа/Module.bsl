...
Процедура ЗаполнитьРеквизитыПриСоздании(ТоварыНакладной)
	...
	ИначеЕсли Параметры.ОрдернаяСхемаПриПоступлении Тогда
		ПоОрдеру = НЕ ПоЗаказам ИЛИ НЕ ЗакупкиСервер.РаспоряжениеНаПриемкуТовараНакладная(Неопределено);
	Иначе
		ПоОрдеру = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуТоваров(ТоварыНакладной)
	...
	Если Параметры.РесурсыЗаполенения <> Неопределено Тогда
		ПараметрыЗаполнения.Вставить("Ресурсы", Параметры.РесурсыЗаполенения);
	КонецЕсли;
