...
Процедура ДобавитьТекстЗапросаПрочиеДоходы(Запрос, ТаблицаДвижений)
	...
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ПереоценкаВалютныхСредств)
...
Процедура ДобавитьТекстЗапросаСебестоимостьТоваров(Запрос, ТаблицаДвижений)
	...
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельности,
	|	ВЫБОР КОГДА Т.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах) 
...
Процедура ДобавитьТекстЗапросаТоварыКОформлениюОтчетовКомитенту(Запрос, ТаблицаДвижений)
	...
	|	ЕСТЬNULL(ВидыЗапасов.УстарелоПодразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК Подразделение,
...
Функция КоличествоРегистраторов()
	...
	Для Каждого ТипРегистратора Из ТипыРегистраторов Цикл
		...
		Если Исключения[МетаданныеРегистратора.Имя] = Неопределено Тогда
			ТекстЗапроса = СтрШаблон(ШаблонЗапроса, МетаданныеРегистратора.Имя, МетаданныеРегистратора.Синоним);
