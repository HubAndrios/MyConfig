...
Функция ПолучитьСостояниеОстатковТоваровОрганизаций(Параметры, АдресХранилища = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ ТаблицаОстатковНаКонецМесяца
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	Остатки.НомерГТД КАК НомерГТД,
	|	СУММА(Остатки.Количество) КАК Количество
	|ПОМЕСТИТЬ ТаблицаАктуальныхОстатков
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТоварыОрганизацийОстаткиИОбороты.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|		ТоварыОрганизацийОстаткиИОбороты.Организация КАК Организация,
	|		ТоварыОрганизацийОстаткиИОбороты.ВидЗапасов КАК ВидЗапасов,
	|		ТоварыОрганизацийОстаткиИОбороты.НомерГТД КАК НомерГТД,
	|		ТоварыОрганизацийОстаткиИОбороты.КоличествоОстаток КАК Количество
	|	ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаОстатковНаКонецМесяца.Период КАК Период,
	|	ТаблицаОстатковНаКонецМесяца.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	ТаблицаОстатковНаКонецМесяца.Организация КАК Организация,
	|	ТаблицаОстатковНаКонецМесяца.ВидЗапасов КАК ВидЗапасов,
	|	ТаблицаОстатковНаКонецМесяца.НомерГТД КАК НомерГТД,
	|	ВЫБОР 
	|		КОГДА ТаблицаОстатковНаКонецМесяца.Количество < 0
	|			И ТаблицаОстатковНаКонецМесяца.Количество < ЕСТЬNULL(ТаблицаАктуальныхОстатков.Количество, 0)
	|			ТОГДА ТаблицаОстатковНаКонецМесяца.Количество
	|		КОГДА ТаблицаОстатковНаКонецМесяца.Количество < 0
	|			И ТаблицаОстатковНаКонецМесяца.Количество >= ЕСТЬNULL(ТаблицаАктуальныхОстатков.Количество, 0)
	|			ТОГДА ЕСТЬNULL(ТаблицаАктуальныхОстатков.Количество, 0)
	|		ИНАЧЕ ТаблицаОстатковНаКонецМесяца.Количество
	|	КОНЕЦ КАК Количество,
	|	ТаблицаОстатковНаКонецМесяца.Оборот КАК Оборот
	|ПОМЕСТИТЬ ТаблицаОстатков
	|ИЗ 
	|	ТаблицаОстатковНаКонецМесяца КАК ТаблицаОстатковНаКонецМесяца
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ТаблицаАктуальныхОстатков КАК ТаблицаАктуальныхОстатков
	|	ПО
	|		ТаблицаОстатковНаКонецМесяца.АналитикаУчетаНоменклатуры = ТаблицаАктуальныхОстатков.АналитикаУчетаНоменклатуры
	|		И ТаблицаОстатковНаКонецМесяца.Организация = ТаблицаАктуальныхОстатков.Организация
	|		И ТаблицаОстатковНаКонецМесяца.ВидЗапасов = ТаблицаАктуальныхОстатков.ВидЗапасов
	|		И ТаблицаОстатковНаКонецМесяца.НомерГТД = ТаблицаАктуальныхОстатков.НомерГТД
	|
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
