...
Процедура УстановитьСнятьПометкиТаблицыТовары(ЗначениеПометки)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.Товары.ВыделенныеСтроки Цикл
		СтрокаТовары = Товары.НайтиПоИдентификатору(ВыделеннаяСтрока);
...
Процедура ПересчитатьИтоги(Форма)
	...
	Итоги = ТаблицаИтогов(Форма.Товары);
	
	Форма.ЗаказыКОформлению.Очистить();
	Для Каждого СтрокаИтогов Из Итоги Цикл
		Если СтрокаИтогов.Количество > 0 Тогда
			ЗаполнитьЗначенияСвойств(Форма.ЗаказыКОформлению.Добавить(), СтрокаИтогов);
		КонецЕсли;
	КонецЦикла;
...
Функция ТаблицаИтогов(Знач Товары)
	...
	ТаблицаИтогов.Свернуть("ПоставщикНаименование", "СуммаСНДС, Количество, СтрокВЗаказе");
			...
			Если ЗначениеЗаполнено(Результат.АдресРезультата)
				...
				Если ТипЗнч(МассивЗаказов) = Тип("Массив") Тогда
					Для Каждого Заказ Из МассивЗаказов Цикл
						...
						НоваяСтрокаЗаказа.ИдентификаторыПредложений = Заказ.ИдентификаторыПредложений
					КонецЦикла;
...
Процедура ЗаполнитьПараметрыФормы()
	...
	ИначеЕсли Параметры.Свойство("МассивКорзины") Тогда
		
		Для Каждого СоставПозиции Из Параметры.МассивКорзины Цикл
			СтрокаКорзины = Товары.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаКорзины, СоставПозиции);
			СоставПозиции.Свойство("ЕдиницаИзмерения", СтрокаКорзины.ЕдиницаИзмеренияНоменклатуры);
			СоставПозиции.Свойство("Характеристика", СтрокаКорзины.ХарактеристикаНоменклатуры);
			СтрокаКорзины.ИспользованиеХарактеристик = ЗначениеЗаполнено(СтрокаКорзины.ХарактеристикаНоменклатуры);
			ТорговыеПредложенияПереопределяемый.ПересчитатьСуммуПоСтроке(СтрокаКорзины);
			СтрокаКорзины.Пометка = СтрокаКорзины.Количество;
		КонецЦикла;
...
Процедура ИзменитьКоличествоВКорзинеПоИдентификатору(ПредложениеИдентификатор, Количество, ЭтоРазница = Ложь)
	...
	Иначе
		...
		СтрокаТовары.СтавкаНДС       = ТорговыеПредложенияПереопределяемый.ЗначениеПеречисленияПоНаименованию(
	...
	Если НоменклатураПоставщиков.Количество() Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаНоменклатурыПоставщиков Цикл
			...
			Для Каждого СтрокаТоваровСНоменклатуройПоставщика Из ТоварыСНоменклатуройПоставщика Цикл
				...
				СтрокаТоваров.НоменклатураПоставщика = СтрокаТаблицы.НоменклатураПоставщика;
...
Функция АдресХранилищаТоваров()
	...
	КорзинаОбъект = РеквизитФормыВЗначение("Товары");
