...
Процедура ОбработкаОповещенияНаСервере(ИмяСобытия)
	...
	ТорговыеПредложенияПереопределяемый.ОбработатьОповещение(ЭтотОбъект, ИмяСобытия);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные.КоличествоПредложений > 0 Тогда
...
&НаКлиенте
Процедура ПоискПредложений(Команда)
	
	ОчиститьСообщения();
	
	Обработчик = Новый ОписаниеОповещения("ПоискПредложенийПродолжение", ЭтотОбъект);
...
&НаКлиенте
Процедура ПоискПредложенийПродолжение(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ЗаполнитьПараметрыПроцедуры(ПараметрыПроцедуры)
	...
	ПараметрыПроцедуры.Вставить("ОграничениеТорговыхПредложений", ОграничениеТорговыхПредложений);
	ПараметрыПроцедуры.Вставить("ПревышениеМинимальнойЦены", ПревышениеМинимальнойЦены);
	ПараметрыПроцедуры.Вставить("МаксимальныйСрокПоставки", МаксимальныйСрокПоставки);
	ПараметрыПроцедуры.Вставить("ПоискПоНаименованию", ПоискПоНаименованию);
	ПараметрыПроцедуры.Вставить("ПоискПоАртикулу", ПоискПоАртикулу);
	ПараметрыПроцедуры.Вставить("ПоискПоШтрихКоду", ПоискПоШтрихКоду);
	ПараметрыПроцедуры.Вставить("ПоискПоХарактеристике", ПоискПоХарактеристике);
	ПараметрыПроцедуры.Вставить("ВалютаКод", 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Валюта, "Код"));
	ПараметрыПроцедуры.Вставить("Товары", Список.Выгрузить());
	ПараметрыПроцедуры.Товары.Индексы.Добавить("Номенклатура, Характеристика, Упаковка");
...
Функция НайтиПредложенияВФоне(Знач Задание, Знач ИдентификаторФормы)
	...
	ЗаполнитьПараметрыПроцедуры(Задание.ПараметрыПроцедуры);
...
&НаСервере
Процедура НайтиПредложенияНаСервере()
	
	ПараметрыПроцедуры = Новый Структура;
	ЗаполнитьПараметрыПроцедуры(ПараметрыПроцедуры);
	АдресРезультата = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
	Попытка
		ТорговыеПредложения.НайтиТорговыеПредложенияПоСпискуТоваров(ПараметрыПроцедуры, АдресРезультата);
	Исключение
		ТекстОшибки = нСтр("ru='Ошибка поиска торговых предложений'");
	...
	ЗагрузитьРезультатыПоиска(АдресРезультата);
...
Процедура ЗагрузитьРезультатыПоиска(АдресРезультата)
	...
	Для Каждого СтрокаРезультата Из ТаблицаПоставщиков Цикл
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы("Цена_"
			+ Идентификатор, ТипЧисло,  "Список", НСтр("ru = 'Цена'")));
	...
	Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
		...
		СтрокаСписка.Наименование = СтрокаРезультата.Наименование;
...
Процедура ОбновитьНадписьПодобраноТоваров(Форма)
	...
	Если КоличествоТоваров = 0 Тогда
		Форма.НадписьПодобраноТоваров = НСтр("ru = 'Отсутствуют товары в корзине'");
	Иначе
		Форма.НадписьПодобраноТоваров = НСтр("ru = 'В корзине %1 поз. из %2 на сумму %3 %4'");
		Форма.НадписьПодобраноТоваров = СтрШаблон(Форма.НадписьПодобраноТоваров, КоличествоТоваров,
			Форма.Список.Количество(), Форма.СуммаВсего, Форма.Валюта);
	КонецЕсли;
...
Процедура ОткрытьФормуОформленияЗаказов()
	...
	Для каждого СтрокаПоиска Из ВыбранныеСтроки Цикл
		...
		МассивКорзины.Добавить(СоставПозиции);
		
	КонецЦикла;
	
	Если МассивКорзины.Количество() = 0 Тогда
	...
	ПараметрыОткрытия.Вставить("МассивКорзины", МассивКорзины);
...
Процедура ИнформацияПоТоваруНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ТорговыеПредложения.Форма.ПоискПредложенийПоСтроке", ПараметрыОткрытия, ЭтотОбъект);
