...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	#Если ВебКлиент Тогда
		ВызватьИсключение НСтр("ru = 'Обработка не предназначена для работы в веб-клиенте.'");
	#КонецЕсли
...
Функция ВсеМесяцыЗакрыты()
	...
	|									ЗНАЧЕНИЕ(Перечисление.ОперацииЗакрытияМесяца.СторноДоходовКУДиРПоЕНВД),
...
Процедура Тестирование()
	...
	Если ТекущийМесяц <= КонецРасчета Тогда
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Активно задание закрытия месяца. Невозможно запустить тестирование.'");
			Сообщение.Сообщить();
		КонецЕсли;
	Иначе
		ИзмененияВИнтерфейсеПриРаботеВФоне(ЗаданиеЗапущено);
...
Функция ЕстьАктивноеФоновоеЗадание()
	...
	Возврат ЗакрытиеМесяцаСервер.ЕстьАктивноеФоновоеЗадание();
...
Функция ОтменитьФоновоеЗаданиеЗакрытияМесяца()
	...
	Идентификатор = ЗакрытиеМесяцаСервер.ИдентификаторАктивногоФоновогоЗадания();
	
	Если НЕ ЗначениеЗаполнено(Идентификатор) Тогда
		Результат = Истина;
	Иначе
		Результат = ЗакрытиеМесяцаСервер.ОтменитьВыполнениеФоновогоЗадания(Идентификатор);
	КонецЕсли;
