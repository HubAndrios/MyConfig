...
Процедура ЗаписатьДокумент(ДанныеСтроки,ДанныеСтрокиТЧтовар)
	...
	док = Документы.пр_ПередачаВмонтаж.НайтиПоНомеру(НомерЗаявки,ДатаОбращения);
	
	Если Не ЗначениеЗаполнено(док) Тогда
		док = Документы.пр_ПередачаВмонтаж.СоздатьДокумент();
	Иначе 
		док = док.ПолучитьОбъект();	
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(док.Состояние) Тогда 
		ДанныеСтроки.Вставить("Состояние",Перечисления.СтатусыПередачаВмонтаж.НеСогласован);
	КонецЕсли;
