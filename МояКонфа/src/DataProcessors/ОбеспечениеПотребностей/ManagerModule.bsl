...
Функция ТаблицаЗапасы(Отбор, ТипОтбора, ТаблицаСпособовОбеспечения = Неопределено) Экспорт
	...
	Запрос.Текст = Запрос.Текст +
		...
		|	&ФОЗаказыНаПеремещенияВключена И НЕ Т.Склад.ЦеховаяКладовая
		|		ИЛИ &ФОПроизводство2_2Включена И Т.Склад.ЦеховаяКладовая КАК ПеремещениеРазрешено,
		|
		...
		|					&ФОЗаказыНаПеремещенияВключена И НЕ Т.Склад.ЦеховаяКладовая
		|					ИЛИ &ФОПроизводство2_2Включена И Т.Склад.ЦеховаяКладовая
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ
		|
