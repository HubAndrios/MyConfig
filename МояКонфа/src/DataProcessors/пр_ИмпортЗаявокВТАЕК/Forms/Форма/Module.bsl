...
Процедура ЗаписатьДокумент(ДанныеСтроки,ДанныеСтрокиТЧтовар)
	...
	докЗаявкаТАЕК = Документы.пр_ЗаявкаВТАЕК.НайтиПоНомеру(НомерЗаявки,ДатаОбращения);
	
	Если Не ЗначениеЗаполнено(докЗаявкаТАЕК) Тогда
		докЗаявкаТАЕК = Документы.пр_ЗаявкаВТАЕК.СоздатьДокумент();
	Иначе 
		докЗаявкаТАЕК = докЗаявкаТАЕК.ПолучитьОбъект();	
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(докЗаявкаТАЕК.Состояние) Тогда 
		ДанныеСтроки.Вставить("Состояние",Перечисления.СтатусыЗаявкиТАЕК.НеСогласован);
	КонецЕсли;
	...
	Строки = докЗаявкаТАЕК.Товары.НайтиСтроки(Новый Структура("КодОборудования,Номенклатура",Артикул,Номенклатура));
	...
	докЗаявкаТАЕК.Записать(РежимЗаписиДокумента.Проведение);				
