...
#Область ПроверкиТабличнойЧастиТовары
	...
	Если Объект.Товары.Итог("КоличествоУпаковок") = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не введено ни одной строки для оформления документов продажи в список ""Товары""'"),
			,
			"Объект.Товары",
			,
			ЕстьОшибкиТовары);
...
Процедура ЗаполнитьУсловияПродаж(Знач УсловияПродаж)
	...
	Иначе
		ПорядокОплаты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ПорядокОплаты");
	КонецЕсли;
...
Процедура ИнициализироватьПомощникПродаж()
	...
	ТаблицаОснованийДляПечати.Загрузить(Обработки.ПомощникПродаж.ПолучитьТаблицуОснованийДляПечати(
...
Процедура УправлениеДоверенностями()
	...
	Для Каждого СтрокаДокумент Из Объект.Документы Цикл 
		Если ТипЗнч(СтрокаДокумент.Документ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
			...
			ЗаказПомеченНаУдаление = СтруктураРезультат.ПометкаУдаления;
			ЗаказПроведен 		 = СтруктураРезультат.Проведен;
...
Процедура ЗаполнитьТоварыПоСоглашениюСерверСЗамером()
	Если Объект.ЗаполнятьТоварыПоСоглашению Тогда
		Замер = Производительность.НачатьЗамерВремени("Обработка.ПомощникПродаж.Форма.Форма.ЗаполнитьТоварыПоСоглашениюСервер");
		КоличествоОбработанных = ЗаполнитьТоварыПоСоглашениюСервер();
		Производительность.ЗакончитьЗамерВремени(Замер, КоличествоОбработанных/10);
		ОбновитьНадписьВсегоПодобраноПозиций(НадписьВсегоПодобраноПозиций, Объект.Товары.Количество());
	КонецЕсли;
КонецПроцедуры
