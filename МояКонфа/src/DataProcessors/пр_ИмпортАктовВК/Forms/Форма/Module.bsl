...
Процедура ЗагрузкаНаКлиентеOLE()
	...
	стЗаписываемыеКолонкиТЧТовар.Вставить("ПолноенаименованиеединицыоборудованиявсоответствиисГОСТ(ОСТ),ТЗ/ТУ/ТТ(чертежом)","Наименование");
	...
	стЗаписываемыеКолонкиТЧТовар.Вставить("Основныетехническиехарактеристики","ТехХарактеристика");
...
Процедура ЗаписатьДокумент(ДанныеСтроки,ДанныеСтрокиТЧтовар,Первичный=Истина)
	...
	НомерДок="";
	АртикулЗаполнен = ДанныеСтроки.Свойство("РегистрационныйНомер",НомерДок);
	ДокАктВК = Документы.пр_АктВК.НайтиПоРеквизиту("РегистрационныйНомер",НомерДок);
	
	Если Не ЗначениеЗаполнено(ДокАктВК) Тогда
		ДокАктВК = Документы.пр_АктВК.СоздатьДокумент();
	Иначе 
		ДокАктВК = ДокАктВК.ПолучитьОбъект();	
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДокАктВК.Состояние) Тогда 
		ДанныеСтроки.Вставить("Состояние",Перечисления.СтатусыАктаВК.НеСогласован);
	КонецЕсли;
	
	ДанныеСтроки.Вставить("Дата",ТекущаяДата());
	ЗаполнитьЗначенияСвойств(ДокАктВК, ДанныеСтроки);
	...
	Строки = ДокАктВК.Товары.НайтиСтроки(Новый Структура("КодОборудования,Номенклатура",Артикул,Номенклатура));
	Если Строки.Количество() > 0 Тогда
		СтрокаТовары = Строки[0];
	Иначе
		СтрокаТовары = ДокАктВК.Товары.Добавить();
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(СтрокаТовары, ДанныеСтрокиТЧтовар);
	
	ДокАктВК.Записать(РежимЗаписиДокумента.Проведение);				
	Сообщить(СокрЛП(ДокАктВК)+" - загружен.");
