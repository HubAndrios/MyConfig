...
Процедура УстановитьОтображениеЗаголовкаИЭлементовУправления()
	...
	КонецЕсли;
	ЗаголовкиСтраниц.Вставить(Элементы.СтраницаРаспределениеПоОрганизациям,	НСтр("ru = 'Распределение по владельцам.'"));
...
Функция ПолучитьТаблицуРаспределенийСервер()
	...
	Для Каждого Строка Из Объект.ТоварыРаспределенные Цикл
		...
		Если Строка.ОперацияТекст = "Порча"
			...
			Если МассивСтрок.Количество() > 0 Тогда
				Если Строка.ОперацияТекст = "Списание" Тогда
					МассивСтрок[0].КоличествоОстатокСУчетомРаспределений =
						МассивСтрок[0].КоличествоОстатокСУчетомРаспределений - Строка.КоличествоРаспределено;
				Иначе
					МассивСтрок[0].КоличествоОстатокСУчетомРаспределений =
						МассивСтрок[0].КоличествоОстатокСУчетомРаспределений - 2 * Строка.КоличествоРаспределено;
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого РаспределяемаяСтрока Из МассивРаспределяемыхСтрок Цикл
		Если РаспределяемаяСтрока.ОперацияТекст = "Оприходование" Тогда
			...
			Для Каждого СтрокаОстаток Из Остатки Цикл
				// Оприходование товаров на ответ. хранение не поддерживается.
				Если ЗначениеЗаполнено(СтрокаОстаток.Партнер) Тогда
					Продолжить;
				КонецЕсли;
		...
		Иначе
			...
			Для Каждого СтрокаОстаток Из Остатки Цикл
				// Порча / пересортица товаров на ответ. хранение не поддерживается
				Если РаспределяемаяСтрока.ОперацияТекст <> "Списание"
					И ЗначениеЗаполнено(СтрокаОстаток.Партнер) Тогда
				...
				Если СтрокаОстаток.КоличествоОстатокСУчетомРаспределений > 0 Тогда
					...
					Если ЗначениеЗаполнено(СтрокаОстаток.Партнер) Тогда
						СтрокаРаспределение.Договор = СтрокаОстаток.Договор;
						СтрокаРаспределение.Партнер = СтрокаОстаток.Партнер;
					КонецЕсли;
					...
					Если РаспределяемаяСтрока.ОперацияТекст = "Списание" Тогда
						СтрокаОстаток.КоличествоОстатокСУчетомРаспределений = СтрокаОстаток.КоличествоОстатокСУчетомРаспределений -
							СтрокаРаспределение.КоличествоРаспределено;
					Иначе
						СтрокаОстаток.КоличествоОстатокСУчетомРаспределений = СтрокаОстаток.КоличествоОстатокСУчетомРаспределений -
							2 * СтрокаРаспределение.КоличествоРаспределено;
					КонецЕсли;
