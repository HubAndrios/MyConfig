	...
	Если ТипЗаказа = "ЗаказКлиента" ИЛИ ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		...
		ИначеЕсли Заказы.Количество() = 1 И ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
			...
			Иначе
				...
				СтруктураРасчетов = Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПолучитьСтруктуруРасчетовПоЗаявке(Заказы[0].Значение);
...
Процедура ЗаполнитьПодсказкиИДекорации()
	...
	Если ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		ТекстТЧОтгрузка = НСтр("ru='заменяющих товаров'");
		ТекстТЧПриемка = НСтр("ru='возвращаемых товаров'");

	ИначеЕсли ТипЗаказа = "ЗаказНаСборку" Тогда
		ТекстТЧОтгрузка = НСтр("ru='списываемых комплектующих / разбираемых комплектов'");
		ТекстТЧПриемка = НСтр("ru='приходуемых комплектующих / собираемых комплектов'");
	КонецЕсли;
	...
	Элементы.СкорректироватьМерныеТовары.Заголовок =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Скорректировать строки мерных товаров %1.'"),
			ТекстТЧОтгрузка);
	...
	Элементы.МерныеТоварыККорректировкеОтгрузкаДекорация.Заголовок =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Строки %1 %2 с мерными товарами, по которым имеются расхождения в пределах допустимых отклонений будут скорректированы.'"),
			ТекстТЧОтгрузка,
			ТекстРП);
	...
	Элементы.ТоварыКОтменеОтгрузкаДекорация.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Строки %1 %2, по которым %3, будут отменены.'"),
		ТекстТЧОтгрузка,
		ТекстРП,
		ТекстНеОформленаОтгрузка);
...
Процедура ЗаполнитьОтклоненияПоЗаказамНаСборку()
	...
	|		0                                                      КАК КОформлениюОстаток,
	|		ОстаткиКПоступлению.КПоступлениюОстаток                КАК ОстатокПоОрдерам,
	|		0                                                      КАК Заказано
	|	ИЗ
