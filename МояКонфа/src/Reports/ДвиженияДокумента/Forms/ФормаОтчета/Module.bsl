...
Процедура СформироватьОтчет()
	...
	МассивРегистров     = ПроведениеСерверУТ.ПолучитьМассивИспользуемыхРегистров(Отчет.Документ, МетаданныеДокумента.Движения);
		...
		Для Каждого Измерение Из МетаданныеРегистра[ИмяКоллекции] Цикл
			...
			ИначеЕсли Измерение.Тип.СодержитТип(Тип("СправочникСсылка.КлючиАналитикиУчетаПартий")) И Измерение.Тип.Типы().Количество() = 1 Тогда
				...
				ТекстПоля = "" + ПсевдонимТаблицы +".ГруппаФинансовогоУчета     КАК " + Измерение.Имя + "ГруппаФинансовогоУчета,
							...
							|" + ПсевдонимТаблицы +".КодСтроки                  КАК " + Измерение.Имя + "КодСтроки,";
				...
				МассивПолей.Добавить(Измерение.Имя + "КодСтроки");
...
Процедура ВывестиДанныеПоРегистру(МетаданныеРегистра, Макет)
	...
	СинонимРегистра = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Регистр %1 ""%2""'"),
		НРег(ВидРегистра),
		?(ЗначениеЗаполнено(МетаданныеРегистра.ПредставлениеСписка), МетаданныеРегистра.ПредставлениеСписка, МетаданныеРегистра.Синоним));
	
	ОбластьРазделитель = Макет.ПолучитьОбласть("Разделитель");
	ОбластьЗаголовок   = Макет.ПолучитьОбласть("ЗаголовокОтчета");
	
	УстановитьПараметрОбласти(ОбластьЗаголовок, "СинонимРегистра", СинонимРегистра);
