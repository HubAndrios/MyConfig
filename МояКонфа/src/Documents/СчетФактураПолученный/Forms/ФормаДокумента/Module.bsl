...
&НаСервереБезКонтекста
Процедура ЗаполнитьСписокВыборкаДатыЗаписиКнигиПокупок(СписокВыбора, Знач ДатаПолучения)
...
Процедура УстановитьДатуЗаписиКнигиПокупок()
	...
	Если Объект.Корректировочный ИЛИ Объект.Исправление Тогда
		Объект.ДатаЗаписиКнигиПокупок = КонецКвартала(Объект.Дата);
	ИначеЕсли Объект.ОтнестиКПредыдущемуОтчетномуКварталу Тогда
