...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазрешатьПродажуАлкогольнойПродукцииБезСопоставленияЕГАИС = РозничныеПродажиВызовСервера.РазрешатьПродажуАлкогольнойПродукцииБезСопоставленияЕГАИС();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен()
		...
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияКлиентПереопределяемый.ЕстьНеобработанноеСобытие() Тогда
			...
			Если Не ИспользоватьАкцизныеМарки
				ИЛИ (ИспользоватьАкцизныеМарки
				     И Не АкцизныеМаркиЕГАИСКлиент.СчитанаАкцизнаяМарка(ЭтотОбъект, ТекущиеДанные, ДанныеШтрихкодов)) Тогда
...
Процедура РасчетСкидокНаКлиенте(ВозвращенноеЗначениеУправляемыеСкидки, ДополнительныеПараметры)
	...
	Если (ИспользоватьАвтоматическиеСкидкиВПродажах
		...
		Если Не Результат.РасчетВыполненУспешно Тогда
			...
			ДополнительныеПараметрыПопыткаЗаписи.Вставить("ТекстСообщения", НСтр("ru = 'При выполнении операции резервирования не удалось записать документ.'"));
			...
			ДополнительныеПараметрыПопыткаЗаписи.Вставить("ИмяПроцедуры", "ВыполнитьЧастьРасчетаНаСервере");
...
Процедура ПечатьЧека_ПослеОшибкиПечатиЧека(ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЕстьАлкогольнаяПродукция Тогда
		...
		ПараметрыВыполнения.ВходныеПараметры.НомерЧека  = ПараметрыВыполнения.ВходныеПараметры.НомерЧека + 1000000;
		ПараметрыВыполнения.ВходныеПараметры.ТипРасчета = ПредопределенноеЗначение("Перечисление.ТипыРасчетаДенежнымиСредствами.ВозвратДенежныхСредств");
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если Не ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования Тогда
			...
			Если РезультатВыполнения.ВыходныеПараметры.Количество() > 5 Тогда
				Объект.АдресЧекаЕГАИС   = РезультатВыполнения.ВыходныеПараметры[4];
				Объект.ПодписьЧекаЕГАИС = РезультатВыполнения.ВыходныеПараметры[5];
			КонецЕсли;
		...
		Если Не ИзмененныеДанныеЗаписаны Тогда
			...
			ДополнительныеПараметрыПовторЗаписи.Вставить("РеквизитыФискальнойОперацииКассовогоУзла", РеквизитыФискальнойОперацииКассовогоУзла);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь)
	...
	Если ИспользоватьАкцизныеМарки И ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияАкцизныхМарокПослеОбработкиШтрихкодов(СтруктураДействий) Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИдентификаторСтроки", СтруктураДействий.МассивСтрокСАкцизнымиМарками[0]);
		ДополнительныеПараметры.Вставить("Редактирование", Ложь);
		ДополнительныеПараметры.Вставить("АдресВоВременномХранилище", "");
		ДополнительныеПараметры.Вставить("Форма", ЭтотОбъект);
		ДополнительныеПараметры.Вставить(
			"ОповещениеПриЗавершении",
			Новый ОписаниеОповещения("ИзменениеАкцизныхМарокЗавершение", ЭтотОбъект, ДополнительныеПараметрыИзменениеАкцизныхМарок));
...
&НаСервере
Функция НайтиСтрокуТЧПоСопоставленнойНоменклатуре(СопоставленнаяНоменклатура)
	
	Результат = Неопределено;
	
	Отбор = Новый Структура;
	Отбор.Вставить("Номенклатура", СопоставленнаяНоменклатура.Номенклатура);
	Отбор.Вставить("Характеристика", СопоставленнаяНоменклатура.Характеристика);
	НайденныеСтроки = Объект.Товары.НайтиСтроки(Отбор);
	Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
		...
		Иначе
			Если СтрокаТЧ.НоменклатураЕГАИС = СопоставленнаяНоменклатура.НоменклатураЕГАИС Тогда
				Результат = СтрокаТЧ.ПолучитьИдентификатор();
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
		...
		Если (СопоставленнаяНоменклатура <> Неопределено
			...
			Иначе
				...
				Если Объект.Товары.Количество() = 0 Тогда
					...
					КонецЕсли;
				Иначе
					ИдентификаторСтроки = НайтиСтрокуТЧПоСопоставленнойНоменклатуре(СопоставленнаяНоменклатура);
...
Процедура ПоискПоШтрихкодуЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	Если Не АкцизныеМаркиЕГАИСКлиент.СчитанаАкцизнаяМарка(ЭтотОбъект, ТекущиеДанные, ДанныеШтрихкодов) Тогда
		ОбработатьШтрихкоды(ДанныеШтрихкодов);
	КонецЕсли;
