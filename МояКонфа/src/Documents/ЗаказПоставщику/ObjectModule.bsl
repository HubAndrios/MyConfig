...
//
Процедура ЗаполнитьХозяйственнуюОперациюПоДоговору(Договор) Экспорт
	
	Если ЗначениеЗаполнено(Договор) Тогда
		ХозяйственнаяОперацияДоговора = ЗакупкиСервер.ХозяйственныеОперацииРаздельнойЗакупки("Документ.ЗаказПоставщику",
			, Договор)[0];
		
		ХозяйственнаяОперация = ЗакупкиСервер.ОсновнаяХозяйственнаяОперацияРаздельнойЗакупки(ХозяйственнаяОперацияДоговора);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ВариантПриемкиТоваров = ?(ЗначениеЗаполнено(Соглашение),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Соглашение, "ВариантПриемкиТоваров"),
...
Функция ОтменитьНепоставленныеСтроки(ПричинаОтмены, Знач ПроверятьОстатки = Ложь) Экспорт
	...
	|	ТоварыКПоступлениюОстатки.КПоступлениюРасход + ТоварыКПоступлениюОстатки.ПринимаетсяПриход КАК Количество
	|ИЗ
