...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса = "
		...
		|				ТОГДА ВЫБОР
		|						КОГДА ТоварыКПоступлениюОстатки.КПоступлениюКонечныйОстаток <= ТоварыКПоступлениюОстатки.КПоступлениюПриход
		|								* (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|							И ТоварыКПоступлениюОстатки.КПоступлениюКонечныйОстаток >= -(ТоварыКПоступлениюОстатки.КПоступлениюПриход
		|								* (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100))
		|							ТОГДА 0 
		|						ИНАЧЕ ЕСТЬNULL(ТоварыКПоступлениюОстатки.КПоступлениюКонечныйОстаток, 0)
		|					КОНЕЦ
		|			ИНАЧЕ ЕСТЬNULL(ТоварыКПоступлениюОстатки.КПоступлениюКонечныйОстаток, 0)
		|		КОНЕЦ                                         КАК КПоступлениюОстаток,
		|		ТоварыКПоступлениюОстатки.КПоступлениюРасход  КАК КПоступлениюРасход
		|
		...
		|	ВЫБОР 
		|		КОГДА ЕСТЬNULL(ТоварыКОформлениюПоступленияОстатки.КОформлениюОстаток,0) > 0 ТОГДА
		|			Истина
		|		ИНАЧЕ
		|			Ложь
		...
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрНакопления.ТоварыКОформлениюПоступления.Остатки(,ДокументПоступления В (&МассивЗаказов)) КАК ТоварыКОформлениюПоступленияОстатки
		|ПО
		|	ДокументЗаказПоставщику.Ссылка = ТоварыКОформлениюПоступленияОстатки.ДокументПоступления
		|ЛЕВОЕ СОЕДИНЕНИЕ
...
Процедура ЗаполнитьПараметрыХозяйственныхОпераций(ПараметрыЗапроса) Экспорт
	...
	ХозОперацииПриемНаОтветственноеХранение = Новый Массив;
	Если ИспользоватьОтветственноеХранениеВПроцессеЗакупки Тогда
		ХозОперацииПриемНаОтветственноеХранение.Добавить(Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи);
	КонецЕсли;
	...
	Если ИспользоватьОтветственноеХранениеВПроцессеЗакупки Тогда
		ХозОперацииНеРазделятьОформлениеЗакупок.Добавить(Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи);
	КонецЕсли;
	...
	ПараметрыЗапроса.ХозОперацииПриемНаОтветственноеХранение = ХозОперацииПриемНаОтветственноеХранение;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ЕСТЬNULL(ЗаказПоставщику.Соглашение.ВариантПриемкиТоваров,
	...
	ХозОперацияТоварыВПутиИНеотфактуровка = Перечисления.ХозяйственныеОперации.ПустаяСсылка();
	Если ЗначениеЗаполнено(Реквизиты.Договор) Тогда
		ОперацииРаздельнойЗакупки = ЗакупкиСервер.ХозяйственныеОперацииРаздельнойЗакупки("Документ.ПоступлениеТоваров",
										Реквизиты.Соглашение, Реквизиты.Договор);
		Если ОперацииРаздельнойЗакупки.Количество() > 0 Тогда
			ХозОперацияТоварыВПутиИНеотфактуровка = ОперацииРаздельнойЗакупки[0];
		КонецЕсли;
	Иначе
	КонецЕсли;
	Запрос.УстановитьПараметр("ХозОперацияТоварыВПутиИНеотфактуровка", ХозОперацияТоварыВПутиИНеотфактуровка);
	...
	Запрос.УстановитьПараметр("ВариантПриемкиТоваров", ?(ЗначениеЗаполнено(Реквизиты.ВариантПриемкиТоваров),
														Реквизиты.ВариантПриемкиТоваров,
...
Процедура ИнициализироватьВспомогательныеПараметрыДокумента(Запрос)
	...
	Если Справочники.СоглашенияСПоставщиками.СоглашениеИспользуетсяПриПриемке(Запрос.Параметры.ВариантПриемкиТоваров)
		...
		// Если приемка не разделена по накладным, то приход по соглашению сделает накладная, а график двигает заказ по соглашению.
		Запрос.УстановитьПараметр("Распоряжение",        Справочники.СоглашенияСПоставщиками.ПолучитьДокументРегистрации(
			Запрос.Параметры.Соглашение));
		Если Запрос.Параметры.ВариантПриемкиТоваров <> Перечисления.ВариантыПриемкиТоваров.НеРазделенаПоНакладным Тогда
...
Функция ТекстЗапросаТаблицаДвижениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|				ЕСТЬNULL(ДокументСоглашение.Ссылка, НЕОПРЕДЕЛЕНО) // Соглашение может быть распоряжением в графике поступления только на ордерных складах.
	|			ИНАЧЕ
	|				&Ссылка
	...
	|ИЗ
	|	Документ.ЗаказПоставщику.Товары КАК ТаблицаТовары
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.СоглашениеСПоставщиком КАК ДокументСоглашение
	|		ПО ТаблицаТовары.Ссылка.Соглашение = ДокументСоглашение.Соглашение
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&ХозОперацияТоварыВПутиИНеотфактуровка КАК ХозяйственнаяОперация
	|ИЗ
	|	Документ.ЗаказПоставщику.Товары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Оформление закупок двумя документами
	|ВЫБРАТЬ
	|	ТаблицаТовары.НомерСтроки              КАК НомерСтроки,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ТаблицаТовары.ДатаПоступления          КАК Период,
	...
	|	И НЕ ТаблицаТовары.Отменено
	|	И &Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден), ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт))
	|	И &ХозОперацияТоварыВПутиИНеотфактуровка <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
	|	И НЕ &НакладнаяЯвляетсяРаспоряжением";
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПереопределениеРасчетаПараметров.Вставить("ВариантПриемкиТоваров",
		"ЕСТЬNULL(ТаблицаТовары.Ссылка.Соглашение.ВариантПриемкиТоваров, &ВариантПриемкиТоваровКонстанта)");
...
Функция ЗаказЯвляетсяРаспоряжениемНаПриемкуТоваров(ЗаказПоставщику) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СоглашенияСПоставщиками.ВариантПриемкиТоваров, НЕОПРЕДЕЛЕНО) КАК ВариантПриемкиТоваров
		|ИЗ
		|	Документ.ЗаказПоставщику КАК ЗаказПоставщику
	...
	Результат = Ложь;
	Если Выборка.Следующий() Тогда
		Если Выборка.ВариантПриемкиТоваров = Неопределено Тогда
			Результат = Константы.ВариантПриемкиТоваров.Получить() = Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным;
		ИначеЕсли Выборка.ВариантПриемкиТоваров = Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным Тогда
			Результат = Истина;
		ИначеЕсли Выборка.ВариантПриемкиТоваров <> Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным Тогда
			Результат = Ложь;
		КонецЕсли;
