...
&НаКлиенте
Процедура ЗакрытьЗаявкуФрагмент(Результат = Неопределено) Экспорт
...
Процедура ОбновитьСостояниеЭлементовУправленияИтоговПоРасчетам()
	...
	Иначе
		...
		СтруктураРасчетов = Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПолучитьСтруктуруРасчетовПоЗаявке(Объект.Ссылка);
...
Функция ОтменитьНевозвращенныеСтрокиСервер(ПроверятьОстатки = Ложь)
	...
	СтруктураКорректировкиСтрок = ЗаказыСервер.ПолучитьСтруктуруКорректировкиСтрокЗаказа();
