...
Процедура ЗаполнитьХозяйственнуюОперациюПоДоговору(Договор) Экспорт
	...
	Если ЗначениеЗаполнено(Договор) Тогда
		ХозяйственныеОперацииДоговора = ЗакупкиСервер.ХозяйственныеОперацииРаздельнойЗакупки(
			"Документ.ПриобретениеТоваровУслуг", , Договор);
		
		Если ХозяйственныеОперацииДоговора.Найти(ХозяйственнаяОперация) = Неопределено Тогда
			ХозяйственнаяОперация = ХозяйственныеОперацииДоговора[0];
		КонецЕсли;
	КонецЕсли;
