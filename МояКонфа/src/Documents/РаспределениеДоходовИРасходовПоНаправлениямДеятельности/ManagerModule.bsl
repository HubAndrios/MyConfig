...
Процедура СоздатьДокументыЗаПериод(Период, Организация = Неопределено, Отказ = Ложь) Экспорт
	...
	ОписаниеЗамера = Производительность.НачатьЗамерВремени("ЗакрытиеМесяца.РаспределениеДоходовИРасходовПоНаправлениямДеятельности");
	...
	Производительность.ЗакончитьЗамерВремени(ОписаниеЗамера, ДокументыРаспределения.Количество());
