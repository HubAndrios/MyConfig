		...
		Если (СопоставленнаяНоменклатура <> Неопределено
			...
			Иначе
				...
				Если Объект.Товары.Количество() = 0 Тогда
					...
					КонецЕсли;
				Иначе
					ТекущиеДанные = НайтиСтрокуТЧПоСопоставленнойНоменклатуре(СопоставленнаяНоменклатура);
					Если ТекущиеДанные = Неопределено Тогда
						ТекущиеДанные = Объект.Товары.Добавить();
						ЗаполнитьЗначенияСвойств(ТекущиеДанные, СопоставленнаяНоменклатура);
						Если ЗначениеЗаполнено(СопоставленнаяНоменклатура.Номенклатура) Тогда
...
Процедура ПечатьЧека_ПослеОшибкиПечатиЧека(ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЕстьАлкогольнаяПродукция Тогда
		...
		ПараметрыВыполнения.ВходныеПараметры.ТипРасчета = ПредопределенноеЗначение("Перечисление.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств");
...
Процедура ОтложитьНаКлиенте(ОповещениеПриЗавершении)
	...
	Если Не Результат Тогда
		...
		ДополнительныеПараметрыПопытка.Вставить("ТекстСообщения", НСтр("ru = 'Не удалось записать документ.'"));
