...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Если Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекаЕГАИСВозврат.ВозвратОтРозничногоПокупателя") Тогда
		Форма.Элементы.НомерЧекаККМ.Заголовок = НСтр("ru = 'Номер чека'");
		Форма.Элементы.НомерСмены.Заголовок = НСтр("ru = 'Номер смены'");
		Форма.Элементы.СерийныйНомерККМ.Заголовок = НСтр("ru = 'Серийный номер ККМ'");
	Иначе
		Форма.Элементы.НомерЧекаККМ.Заголовок = НСтр("ru = 'Номер чека'");
