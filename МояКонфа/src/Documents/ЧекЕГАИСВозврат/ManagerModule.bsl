...
Функция ПараметрыПередачиДанныхЕГАИС(ДокументСсылка) Экспорт
	...
	|	ЧекЕГАИСВозврат.НомерСмены,
	|	ЧекЕГАИСВозврат.НомерЧекаККМ,
	|	ЧекЕГАИСВозврат.СерийныйНомерККМ,
	|	ЧекЕГАИСВозврат.ОрганизацияЕГАИС,
	|	ЧекЕГАИСВозврат.Ответственный.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ЧекЕГАИСВозврат.ОрганизацияЕГАИС.Код КАК ИдентификаторФСРАР
	|ИЗ
	|	Документ.ЧекЕГАИСВозврат КАК ЧекЕГАИСВозврат
	|ГДЕ
	|	ЧекЕГАИСВозврат.Ссылка = &ДокументСсылка");
