...
Процедура ЗаполнитьНаличныеДС(Команда)
	...
	Если НЕ Объект.СуммаДокумента = 0 Тогда
		ТекстВопроса = НСтр("ru='Сумма к получению по доверенности будет перезаполнена по расчетам с клиентом. Продолжить?'");
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьНаличныеДСЗавершение", ЭтотОбъект), ТекстВопроса, СписокКнопок,, КодВозвратаДиалога.Да, НСтр("ru = 'Заполнение суммы к получению'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьНаличныеДСФрагмент()
	...
	Если Объект.СуммаДокумента = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Отсутствует задолженность по расчетам по основанию %ПредставлениеЗаказа%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеЗаказа%", СокрЛП(Объект.ДокументОснование));
