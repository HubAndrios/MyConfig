...
&НаКлиенте
Процедура СтрокаДокументыОплатыНалогаНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Оповещение = Новый ОписаниеОповещения("ДокументыОплатыНалогаПриИзменении", ЭтаФорма);
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("АдресВХранилище", ПоместитьДокументыОплатыНалогаВХранилище());
	ОткрытьФорму("Документ.ТаможеннаяДекларацияИмпорт.Форма.ФормаДокументыОплатыНалога", 
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	СформироватьСтрокуДокументыОплатыНалога();
	ОбновитьИнформациюПоПоступлениям();
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.СтрокаДокументыОплатыНалога.Видимость = Не РасчетыСТаможней;
...
&НаКлиенте
Процедура ДокументыОплатыНалогаПриИзменении(АдресВХранилище, ДополнительныеПараметры) Экспорт
	
	Если АдресВХранилище = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДокументыОплатыНалогаПриИзмененииНаСервере(АдресВХранилище);
	
КонецПроцедуры

&НаСервере
Процедура ДокументыОплатыНалогаПриИзмененииНаСервере(АдресВХранилище)
	
	Объект.ДокументыОплатыНалога.Загрузить(ПолучитьИзВременногоХранилища(АдресВХранилище));
	СформироватьСтрокуДокументыОплатыНалога();
	
КонецПроцедуры

&НаСервере
Функция ПоместитьДокументыОплатыНалогаВХранилище()
	
	Возврат ПоместитьВоВременноеХранилище(Объект.ДокументыОплатыНалога.Выгрузить());
	
КонецФункции

&НаСервере
Процедура СформироватьСтрокуДокументыОплатыНалога()
	
	СтрокаНомеровИДата = "";
	Для Каждого СтрокаТаблицы Из Объект.ДокументыОплатыНалога Цикл
		СтрокаНомеровИДата = СтрокаНомеровИДата + ?(ПустаяСтрока(СтрокаНомеровИДата), "", ", ")
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 от %2'"), СтрокаТаблицы.НомерДокумента, Формат(СтрокаТаблицы.ДатаДокумента, "ДЛФ=D"));
	КонецЦикла; 
	
	СтрокаДокументыОплатыНалога = 
		?(ЗначениеЗаполнено(СтрокаНомеровИДата), СтрокаНомеровИДата, НСтр("ru='<отсутствуют>'"));
