	...
	Для НомерЛиста = 1 По КоличествоСтраниц Цикл 
		...
		Для НомерСтроки = 11 По КоличествоСтрок Цикл 
			...
			Если Не ЕстьОшибки Тогда
				...
				ИДработы="";
				ДанныеСтроки.Свойство("ИДработы",ИДработы);
				НомерБлока="";
				ДанныеСтроки.Свойство("НомерБлока",НомерБлока);
	            Работа = НайтиРаботу(ИДработы,НомерБлока);
				...
				ДанныеСтроки.Вставить("флРабота",Не ЗначениеЗаполнено(Работа));
				...
				ДанныеСтроки.Свойство("флРабота",Флаг); Если не Флаг и Строки.Количество() > 0  Тогда ДанныеСтроки.Вставить("флРабота", Не ЗначениеЗаполнено(Работа)) КонецЕсли;				
...
Функция ПолучитьСтатусНеСогласован()
	возврат Перечисления.СтатусыЛота.НеСогласован;
КонецФункции
...
Функция НайтиРаботу(ИДработы,НомерБлока)
	Возврат ОбщегоНазначения_аккую.НайтиРаботуВГрафике(ИДработы,НомерБлока);	
КонецФункции
