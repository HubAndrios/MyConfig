...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	Строки.СуммаУпр                         КАК СуммаБезНДС,
	|	Строки.СуммаУпр                         КАК СуммаБезНДСУпр,
	|	Строки.СуммаРегл                        КАК СуммаСНДСРегл,
	|	Строки.СуммаРегл                        КАК СуммаБезНДСРегл,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументОбъект.Модифицированность() Тогда
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось заполнить реквизиты в документе ""%1"" по причине: %2'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЗаписьЖурналаРегистрации(
					ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
					УровеньЖурналаРегистрации.Предупреждение,
