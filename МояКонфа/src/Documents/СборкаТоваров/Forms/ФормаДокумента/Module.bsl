...
Процедура НазначениеПриИзменении(Элемент)
	...
	Если ТреубетсяОбновитьНазначениеКомплектующих() Тогда
		
		ОбновитьНазначениеКомплектующихПоНазначениюКомплекта();
...
Процедура ПодобратьТовары(Команда)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СборкаТоваров")
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	РеквизитыШапки = Новый Структура("Организация, Подразделение, Склад, Сделка,
		|ХозяйственнаяОперация, НаправлениеДеятельности, Ссылка");
...
Процедура ОткрытьПодборТоваровПоЗаказамОрдерам()
	...
	РеквизитыШапки = Новый Структура("Организация, Подразделение, Склад, Сделка,
		|ХозяйственнаяОперация, НаправлениеДеятельности, Ссылка");
	ЗаполнитьЗначенияСвойств(РеквизитыШапки, Объект);
	РесурсыЗаполенения = Новый Массив;
	РесурсыЗаполенения.Добавить("Комплект");
	РесурсыЗаполенения.Добавить("Комплектующие");
	...
	ПараметрыФормы.Вставить("РесурсыЗаполенения",                       РесурсыЗаполенения);
...
&НаКлиенте
Функция ТреубетсяОбновитьНазначениеКомплектующих()
	
	ЭтоСборкаБезЗаказа = Не ЗначениеЗаполнено(Объект.ЗаказНаСборку)
		И Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СборкаТоваров");
	
	Возврат ЭтоСборкаБезЗаказа;
	
КонецФункции

&НаСервере
Процедура ОбновитьНазначениеКомплектующихПоНазначениюКомплекта()
	...
	СвойстваНазначенияОбъекта = СвойстваНазначений.Получить(Объект.Назначение);
	Если СвойстваНазначенияОбъекта <> Неопределено И СвойстваНазначенияОбъекта.ЭтоНазначениеНаправленияДеятельности Тогда
		НазначениеПоУмолчанию = Объект.Назначение;
	Иначе
		НазначениеПоУмолчанию = Справочники.Назначения.ПустаяСсылка();
	КонецЕсли;
	НакладныеСервер.ЗаполнитьНазначенияВТабличнойЧасти(Объект.Товары, НазначениеПоУмолчанию);
...
Процедура ОтобразитьСкрытьНаправлениеДеятельности()
	...
	Элементы.НаправлениеДеятельности.Видимость = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров;
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.ТоварыВедетсяУчетПоГТД.Видимость = ВедетсяУчетПоГТД ИЛИ ЭтоРазборка;
