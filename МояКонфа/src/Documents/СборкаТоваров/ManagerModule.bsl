...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ЗначенияПараметров.Вставить("НакладнаяЯвляетсяРаспоряжением",
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	|			Остатки.Серия               КАК Серия
	|		ИЗ
	|			РегистрНакопления.ТоварыКОформлениюПоступления.Остатки(, ТИПЗНАЧЕНИЯ(ДокументПоступления) В(&ДоступныеТипы)) КАК Остатки
	|	ГДЕ
	|		Остатки.КОформлениюОстаток <> 0) КАК ТоварыРаспоряжений) КАК Набор";
...
//	заполнение ТЧ ВидыЗапасовОприходование.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ПО (ВидыЗапасов.Ссылка = СборкаТоваров.Ссылка)
	|ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	КОбработке.Ссылка.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ДокОбъект.ВидыЗапасовОприходование.Количество() = 0 Тогда
			
			ДокументИзменен = Истина;
			
			Если ВыборкаПоДокументам.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров Тогда
				...
				МестаУчета = РегистрыСведений.АналитикаУчетаНоменклатуры.МестаУчета(
					Перечисления.ХозяйственныеОперации.СборкаТоваров, ДокОбъект.Склад, ДокОбъект.Подразделение, Неопределено);
				...
				Если ЗначениеЗаполнено(ДокОбъект.НоменклатураОсновногоКомпонента) И ВыборкаПоДокументам.КомплектВестиУчетПоГТД Тогда
					Для каждого СтрокаВидыЗапасов Из ДокОбъект.ВидыЗапасовСписание Цикл
						Если СтрокаВидыЗапасов.УдалитьКоличествоКомплекта <> 0 Тогда
							НоваяСтрокаЗапаса = ДокОбъект.ВидыЗапасовОприходование.Добавить();
							НоваяСтрокаЗапаса.ВидЗапасов = ДокОбъект.УдалитьВидЗапасов;
							...
							НоваяСтрокаЗапаса.АналитикаУчетаНоменклатуры = ДокОбъект.АналитикаУчетаНоменклатуры;
				...
				Иначе
					НоваяСтрокаЗапаса = ДокОбъект.ВидыЗапасовОприходование.Добавить();
					НоваяСтрокаЗапаса.ВидЗапасов = ДокОбъект.УдалитьВидЗапасов;
					НоваяСтрокаЗапаса.НомерГТД   = ДокОбъект.УдалитьНомерГТД;
					НоваяСтрокаЗапаса.Количество = ДокОбъект.Количество;
					НоваяСтрокаЗапаса.АналитикаУчетаНоменклатуры = ДокОбъект.АналитикаУчетаНоменклатуры;
				КонецЕсли;
				
				ДокОбъект.УдалитьНомерГТД = Неопределено;
				ДокОбъект.УдалитьВидЗапасов = Неопределено;
