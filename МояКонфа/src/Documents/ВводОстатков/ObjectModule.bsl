...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если (ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТоваровПолученныхНаКомиссию
			ИЛИ ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров
			...
			И
			ЗначениеЗаполнено(Организация) И НЕ ЗначениеЗаполнено(НалогообложениеНДС) Тогда
			НалогообложениеНДС = Справочники.Организации.НалогообложениеНДС(Организация, ?(ЗначениеЗаполнено(Склад), Склад, Неопределено), Дата);
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СчетаФактуры.Количество() = 0 И НалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию
...
Процедура СоздатьДокументыРасчетовСПартнерами(ИмяТабличнойЧасти, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ЭтотОбъект[ИмяТабличнойЧасти] Цикл
		...
		Если (Не ЗначениеЗаполнено(СтрокаТаблицы.РасчетныйДокумент) И СтрокаТаблицы.РасчетныйДокумент <> Неопределено)
			 ...
			 ИЛИ ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) = Тип("ПеречислениеСсылка.ТипыДоговоровМеждуОрганизациями") Тогда
			...
			Если Отказ ИЛИ
				...
				И ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) <> Тип("ПеречислениеСсылка.ТипыДоговоров")
				И ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) <> Тип("ПеречислениеСсылка.ТипыДоговоровМеждуОрганизациями"))
			Тогда
				Продолжить;
			КонецЕсли;
			...
			ИначеЕсли ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")
				ИЛИ ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) = Тип("ПеречислениеСсылка.ТипыДоговоровМеждуОрганизациями") Тогда
				...
				НовыйОбъект.ТипДоговора 		 = СтрокаТаблицы.РасчетныйДокумент;
				НовыйОбъект.ВалютаВзаиморасчетов = СтрокаТаблицы.ВалютаВзаиморасчетов;
				НовыйОбъект.ПорядокОплаты		 = Документы.ВводОстатков.ПорядокОплатыДоговораПоВалюте(СтрокаТаблицы.ВалютаВзаиморасчетов);
				НовыйОбъект.Статус				 = Перечисления.СтатусыДоговоровКонтрагентов.Действует;
				НовыйОбъект.ПорядокРасчетов		 = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов;
