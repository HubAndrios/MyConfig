...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		...
		Если Объект.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров Тогда
			Объект.НалогообложениеНДС = Справочники.Организации.ЗакупкаПодДеятельность(Объект.Организация, Объект.Склад, Объект.Дата)
		ИначеЕсли Объект.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТоваровПереданныхНаКомиссию
				ИЛИ Объект.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиМатериаловПереданныхПереработчикам Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы["СобственныеТоварыКонтрагент"].Имя);
	ГруппаОтбора = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(Элемент.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "Объект.Контрагент", Неопределено, ВидСравненияКомпоновкиДанных.НеЗаполнено);
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "Объект.НалогообложениеНДС",
		Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию,
...
Процедура ЗаказПереработчикуПриИзмененииСервер(ОчищатьСтроки)
	...
	|	Т.ЗакупкаПодДеятельность КАК НалогообложениеНДС,
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ОбновитьНалогообложениеНДС(Ложь);
...
&НаСервере
Процедура ОбновитьНалогообложениеНДС(Перезаполнять = Истина)
	...
	Если ЭтоОстаткиСобственныхТоваров Тогда
		...
		Если Перезаполнять Тогда
			Объект.НалогообложениеНДС = ВидДеятельностиНДСОрганизации;
		КонецЕсли;
		
		Если ИспользоватьРаздельныйУчетНДС Тогда
			УчетНДСУТ.ЗаполнитьСписокВыбораДеятельностиНДСПоступления(
				Элементы.СобственныеТоварыНалогообложениеНДС,  
				Объект.Дата,
		...
		Иначе
			ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы, 
				"СобственныеТоварыНалогообложениеНДС", 
				"Видимость", 
				Ложь);
		КонецЕсли;
	...
	ИначеЕсли ЭтоОстаткиПринятойТары Тогда
		...
		Если ИспользоватьРаздельныйУчетНДС Тогда
			УчетНДСУТ.ЗаполнитьСписокВыбораДеятельностиНДСПоступления(
				Элементы.ПринятаяТараНалогообложениеНДС,  
				Объект.Дата,
				ВидДеятельностиНДСОрганизации,
				Перечисления.ХозяйственныеОперации.ОприходованиеТоваров);
		Иначе
			ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы, 
				"ПринятаяТараНалогообложениеНДС", 
				"Видимость",
				Ложь);
		КонецЕсли;
	...
	ИначеЕсли ЭтоОстаткиПереданныхВПроизводствоТоваров Тогда
		...
		Если ИспользоватьРаздельныйУчетНДС Тогда
			УчетНДСУТ.ЗаполнитьСписокВыбораДеятельностиНДСПотребления(
				Элементы.ПереданныеВПроизводствоТоварыНалогообложениеНДС, 
				Объект.Организация,
				Объект.Дата);
		Иначе
			ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы, 
				"ПереданныеВПроизводствоТоварыНалогообложениеНДС", 
				"Видимость", 
				Ложь);
		КонецЕсли;
		
	ИначеЕсли ЭтоОстаткиПереданныхНаКомиссиюТоваров И Перезаполнять Тогда
	...
	ИначеЕсли ЭтоОстаткиПринятыхНаКомиссиюТоваров И Перезаполнять Тогда
	...
	ИначеЕсли ЭтоОстаткиПереданныхПереработчикамТоваров Тогда
		...
		ВидДеятельностиНДСОрганизации = Справочники.Организации.ЗакупкаПодДеятельность(
			Объект.Организация,
			Объект.Склад,
			Объект.Дата);
		...
		Если ИспользоватьРаздельныйУчетНДС Тогда
			УчетНДСУТ.ЗаполнитьСписокВыбораДеятельностиНДСПоступления(
				Элементы.ПереданныеПереработчикамТоварыНалогообложениеНДС,
				Объект.Дата,
				ВидДеятельностиНДСОрганизации,
				Перечисления.ХозяйственныеОперации.ОприходованиеТоваров);
		Иначе
			ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы,
				"ПереданныеПереработчикамТоварыНалогообложениеНДС",
				"Видимость",
				Ложь);
		КонецЕсли;
