...
Функция ВременныеТаблицыДанныхДокумента() Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ВЫБОР
		|		КОГДА ТаблицаТоваров.СтатусУказанияСерийНаСкладах = 14
...
Процедура ЗаполнитьУсловияЗакупок(Знач УсловияЗакупок) Экспорт
	...
	Если УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов <> Неопределено
		И УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов
		И ЗначениеЗаполнено(Договор) Тогда
		
		ЗаполнитьХозяйственнуюОперациюПоДоговору(Договор);
		
	Иначе
		ХозяйственнаяОперация = ЗакупкиСервер.ХозяйственныеОперацииРаздельнойЗакупки("Документ.ПоступлениеТоваров",
			Соглашение)[0];
	КонецЕсли;
...
//
Процедура ЗаполнитьХозяйственнуюОперациюПоДоговору(Договор, ВариантОформленияЗакупок = Неопределено) Экспорт
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ДокументОснование") Тогда
			...
			ДанныеЗаполнения.Свойство("ИмяРегистраЗаполнения", ИмяРегистраЗаполнения);
...
Процедура ЗаполнитьДокументНаОснованииСоглашенияСПоставщиком(Знач ДокументОснование, Знач СкладПоступления = Неопределено, Знач ХозяйственнаяОперацияПриемки = Неопределено, Знач ИмяРегистраЗаполнения = Неопределено)
	...
	Запрос = Новый Запрос(
		...
		|	И &ИмяРегистраЗаполнения = ""ТоварыКОформлениюПоступления""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ТоварыКПоступлениюОстатки.КОформлениюОстаток КАК Количество,
		|	ТоварыКПоступлениюОстатки.КОформлениюОстаток КАК КоличествоУпаковок
		|ИЗ
		...
		|ГДЕ
		|	ТоварыКПоступлениюОстатки.КОформлениюОстаток > 0
		|	И &ИмяРегистраЗаполнения = ""ТоварыКПоступлению""
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ИмяРегистраЗаполнения", ИмяРегистраЗаполнения);
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПоставщику(Знач ДокументОснование, Знач СкладПоступления = Неопределено, Знач ХозяйственнаяОперацияПриемки = Неопределено, Знач ИмяРегистраЗаполнения = Неопределено)
...
Процедура ЗаполнитьДокументНаОснованииПриобретенияТоваров(Знач ДокументОснование, Знач СкладПоступления = Неопределено, Знач ХозяйственнаяОперацияПриемки = Неопределено, Знач ИмяРегистраЗаполнения = Неопределено)
	...
	Запрос = Новый Запрос(
		...
		|	ТоварыКПоступлениюОстатки.КОформлениюОстаток,
		|	ТоварыКПоступлениюОстатки.КОформлениюОстаток
		|ИЗ
