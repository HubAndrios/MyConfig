...
Процедура ЗагрузкаНаКлиентеЕксель()
	...
	стЗаписываемыеКолонки.Вставить("TEC","ТехХарактеристика");
	стЗаписываемыеКолонки.Вставить("KOLVO","Количество");
	...
	Для НомерЛиста = 1 По КоличествоСтраниц Цикл 
		...
		Для НомерСтроки = 3 По КоличествоСтрок Цикл 
			...
			//Если НомерДок=Объект.Номер Тогда
			//	ТекстВопроса = НСтр("ru = 'Вы загружаете документ повторно! "+Символы.ПС
			//	+"Данные в документе будут перезаписаны.Продолжить?'");
			//	
