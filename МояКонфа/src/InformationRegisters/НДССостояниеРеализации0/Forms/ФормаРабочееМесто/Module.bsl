...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.СписокСчетФактураНаНеподтвержденнуюРеализацию0 Тогда
		ОткрытьТекущийСчетФактуруНаНеподтвержденнуюСтавку0();
	ИначеЕсли Поле = Элементы.ТаможеннаяДекларация Тогда
...
Процедура ПодтверждениеСтавкиНДССервер(ПараметрыПодтверждения)
	...
	Попытка
		...
		Если ТекущееСостояние = Перечисления.НДССостоянияРеализация0.НеПодтвержденаРеализация0 Тогда
			ВыписатьСчетФактуруНаНеподтвержденныеСтавки(СчетФактура, Регистратор, ОрганизацияРеализации, ДатаПодтверждения); 
		КонецЕсли;
		Если ПерезаписатьДокумент Тогда
			ИмяДокумента = Регистратор.Метаданные().Имя;
			Документы[ИмяДокумента].СформироватьЗаписиКнигиПродаж(Регистратор, ДатаПодтверждения, ОрганизацияРеализации);
		КонецЕсли;
...
Процедура УстановитьТекстЗапросаСписок()
	...
	|		ПО НДССостояниеРеализации0.Регистратор = СчетФактураВыданный.ДокументОснование
