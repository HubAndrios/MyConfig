...
Процедура ПередУдалениемЭлементаДопРеквизитовСведений(Отказ, ТипЭлемента, ТипВладельцаНабораЭлементов)
	...
	Если ТекущиеДанные.ОбщееСвойство Тогда
		...
		ТекстСообщения = НСтр("ru = '%1 %2 ""%3"" является общим. Состав общих дополнительных %4 можно редактировать только в форме элемента набора общих %4.'");
...
Процедура НастроитьФорму()
	...
	Элементы.СырьевойТовар.Видимость = ЭтоТовар
										И ИспользоватьПродажиНаЭкспортСырьевыхТоваровУслуг;
	#КонецОбласти
...
&НаСервере
Процедура ПометитьТекущееСвойствоНаУдаление(Свойство, ИндексКартинки, ТипВладельцаНабораЭлементов, ТипЭлемента, Отказ)
	
	СвойствоПомеченоНаУдаление = Истина;
	НачатьТранзакцию();
	...
	Исключение
		...
		ОтменитьТранзакцию();
		СвойствоПомеченоНаУдаление = Ложь;
		
		ТекстСообщения = НСтр("ru = 'Не удалось пометить на удаление %1 %2 ""%3"".'");
		
		Параметр1 = ?(ТипЭлемента = "ДопРеквизит", "дополнительный", "дополнительное");
		Параметр2 = ?(ТипЭлемента = "ДопРеквизит", "реквизит", "сведение");
		Параметр3 = Свойство;
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Параметр1, Параметр2, 
			Параметр3);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Пометка на удаление дополнительных свойств элемента.'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка, , ,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
	КонецПопытки;
	
	Если СвойствоПомеченоНаУдаление Тогда
