...
Функция ХозяйственнаяОперация(ТипДоговора) Экспорт
	...
	Возврат
		...
		?(ТипДоговора = Типы.ВвозИзЕАЭС, Операции.ЗакупкаВСтранахЕАЭС,
		Неопределено))))));
КонецФункции
...
Функция ТипДоговора(ХозяйственнаяОперация) Экспорт
	...
	Возврат
		...
		?(ХозяйственнаяОперация = Операции.ЗакупкаВСтранахЕАЭС, Типы.ВвозИзЕАЭС,
		Неопределено))))));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеСправочика = Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				...
				НазначениеОбъект = Неопределено;
				
				
				Если ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС
					И ДанныеОбъекта.ТипДоговора <> Перечисления.ТипыДоговоров.ВвозИзЕАЭС Тогда
					
					ДанныеОбъекта.ТипДоговора = Перечисления.ТипыДоговоров.ВвозИзЕАЭС;
					
				КонецЕсли;
				
				Если ДанныеОбъекта.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.ПустаяСсылка() Тогда
					ДанныеОбъекта.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеРазделять;
				КонецЕсли;
				
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДанныеОбъекта);
				Если НазначениеОбъект <> Неопределено Тогда
					ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НазначениеОбъект);
				КонецЕсли;
