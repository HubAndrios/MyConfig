...
Функция ТаблицаПереименованияДляТекущейВерсии() Экспорт
	...
	ИнтеграцияСтандартныхПодсистем.ПриДобавленииПереименованийОбъектовМетаданных(Итог);
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого ОписаниеОшибки Из Ошибки Цикл
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				ПредставлениеКоличества = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
					КоличествоОшибок - НомерОшибки,
					НСтр("ru = 'ошибка,ошибки,ошибок'"));
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '... И еще %1'"),
...
Функция ИдентификаторыИзКлючей()
	...
	Пока Выборка.Следующий() Цикл
		КлючОбъектаМетаданных = Выборка.КлючОбъектаМетаданных;
