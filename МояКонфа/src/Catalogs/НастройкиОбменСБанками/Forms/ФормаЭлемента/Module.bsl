...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
		...
		Если ЗначениеЗаполнено(Объект.ИмяВнешнегоМодуля) Тогда
			...
			ПараметрыВнешнейКомпоненты = ДополнительныеВнешниеКомпонентыВызовСервера.ПараметрыВнешнейКомпоненты("VPNKeyTLS");
...
Процедура ПослеЗагрузкиФайлаВнешнейКомпонентыСбербанка(Адрес)
	...
	Если ИнформацияОВКБанка.ИмяМодуля <> "VPNKeyTLS" Тогда
		ТекстСообщения = НСтр("ru = 'Выбранный файл содержит другой внешний модуль.
									|Необходимо выбрать файл для Сбербанка.'");
		Возврат;
	КонецЕсли;
	...
	ДополнительныеВнешниеКомпонентыКлиент.ПодключитьВнешнююКомпонентуПоИдентификатору(Оповещение, "VPNKeyTLS", Ложь);
