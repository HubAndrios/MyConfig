...
Процедура ЗаполнитьРеквизитыПоВидуНоменклатуры(Номенклатура,
	...
	+"ИмпортнаяАлкогольнаяПродукция,ОбособленнаяЗакупкаПродажа,СтавкаНДС,ЦеноваяГруппа,СырьевойТовар";
...
Функция ТабличныйДокументКарточкиНоменклатуры(Номенклатура, НастройкиВидимостиИЗаголовков) Экспорт
	...
	Если ГлаваВидна("РегламентированныйИФинансовыйУчет", СтруктураВидимости) Тогда 
		...
		Если РазделВиден("СырьевойТовар", СтруктураВидимости) Тогда 
			Область = Макет.ПолучитьОбласть("СырьевойТоварСтрока");
			Картинка = КартинкаФлагаПоЗначению(Объект.СырьевойТовар);
			Область.Область("СырьевойТоварФлаг").Картинка = Картинка;
			Область.Параметры.Расшифровка = "СырьевойТовар";
			ТабличныйДокументПраво.Вывести(Область);		
		КонецЕсли;
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчет = ВариантыОтчетовУТПереопределяемый.ДобавитьКомандуГотовностьОборудования(КомандыОтчетов);
	Если КомандаОтчет <> Неопределено Тогда
		КомандаОтчет.Важность = "СмТакже";
		КомандаОтчет.РежимЗаписи = "";
	КонецЕсли;
...
Функция РеквизитыОтключенныеПоФО(ЭтоРеквизитыХарактеристик = Ложь) Экспорт
	...
	Если Не ИспользоватьПродажиНаЭкспортСырьевыхТоваровУслуг Тогда
		Результат.Добавить("СырьевойТовар");
	КонецЕсли;
...
Функция ЗависимостьРеквизитовОтТипаНоменклатуры(ТипНоменклатуры,
	...
	НоваяСтрока = ТаблицаРеквизитов.Добавить();
	НоваяСтрока.Реквизит            = Реквизиты.СырьевойТовар;
	НоваяСтрока.Использование = ?(ЭтоТовар,
										Истина,
										Ложь);
...
Функция ТаблицаНастроекРеквизитов(ВидНоменклатуры,
	...
	ВсеДополнительныеРеквизиты(ВидНоменклатуры, "Номенклатура", МенеджерВременныхТаблиц);
