...
&НаСервереБезКонтекста
Процедура СписокСтандартныйПоискНоменклатураПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	//Запрос.Текст = 
	//	"ВЫБРАТЬ
	//	|	ТЗ.Регистратор КАК ВидДокумента,
	...
	//	|	ВЫБОР
	//	|		КОГДА ПоследовательностьПроцессныхСтатусов.СтатусОК = ТЗСОСТОЯНИЙ.Состояние
	//	|			ТОГДА 3
	//	|		ИНАЧЕ 1
	//	|	КОНЕЦ КАК ЗначениеСостояния
	//	|ИЗ
	//	|	РегистрСведений.ПоследовательностьПроцессныхСтатусов КАК ПоследовательностьПроцессныхСтатусов
	...
	//		Минимум = Мин(Минимум,ВыборкаДетальныеЗаписи.ЗначениеСостояния);
	//		Максимум = Макс(Максимум,ВыборкаДетальныеЗаписи.ЗначениеСостояния);
	//	КонецЦикла;
	//	
	//	СтатусОборудования="";
...
Запрос.УстановитьПараметр("Товар",строки.ПолучитьКлючи());

Выборка = Запрос.Выполнить().Выбрать();
Пока  Выборка.Следующий() Цикл
	строкаСписка = Строки[Выборка.Товар];
	строкаСписка.Данные["Состояние"] = Выборка.Состояние;
КонецЦикла;

	
	//	Если Элемент.Колонки.Состояние.Видимость Тогда
		//ОформлениеЯчейки = Элемент.Значение.Оформление["Состояние"];
		//ОформлениеЯчейки.УстановитьЗначениеПараметра("Текст", "222");

   
 //   Для каждого Элемент Из Строки Цикл
	  //  Состояние=ОбщегоНазначения_аккую.ПолучитьСостояниеОборудования(Элемент.Значение.Данные["Ссылка"]);
	  ////  Ссылка = Элемент.Значение.Данные["Организация"];
	  // Сообщить(Элемент); 
	  //  ОформлениеЯчейки = Элемент.Значение.Оформление["Состояние"];
	  //  ОформлениеЯчейки.УстановитьЗначениеПараметра("Текст", Состояние);
        
 //   КонецЦикла; 	

	//Для Каждого ОформлениеСтроки ИЗ ОформленияСтрок Цикл
	//	Если НЕ ОформлениеСтроки.ДанныеСтроки.ЭтоГруппа Тогда
	//		Состояние=ОбщегоНазначения_аккую.ПолучитьСостояниеОборудования(Строки.ТекущийЭлемент);

	//        Если Состояние <> Неопределено Тогда
	//            ОформлениеСтроки.Ячейки.Состояние.УстановитьТекст(Состояние);
	//        КонецЕсли;    
	//    КонецЕсли;    
	//КонецЦикла;    
//  КонецЕсли;
...
//САМ
	Для каждого Элемент Из Строки Цикл
		...
		Элемент.Значение.Данные["Состояние"]=Состояние;
		
		Если Элемент.Значение.Оформление.Получить("Состояние")<>Неопределено Тогда
		ОформлениеЯчейки = Элемент.Значение.Оформление["Состояние"];
		Если СокрЛП(Состояние)="Готово" Тогда
			Цвет = ЦветаСтиля.ДобавленныйРеквизитФон
		ИначеЕсли СокрЛП(Состояние)="В процессе" Тогда
			Цвет = ЦветаСтиля.ЦветДополнительнойНавигации;
		Иначе 
			Цвет = ЦветаСтиля.ОшибкаПолнотекстовыйПоискФон;
		КонецЕсли;
		
		ОформлениеЯчейки.УстановитьЗначениеПараметра("ЦветФона", Цвет);
		КонецЕсли;
